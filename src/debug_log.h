...
private:
  ...
  std::string log_file = "rw_server_" + std::to_string(state_theta_) + "_" +
                         std::to_string(block_size_) + "_" +
...
#ifdef PRINT_LOG
    logger->info("[WRITE PLAN TO STATE][plan node: {}][plan size: {}][sql: {}]",
                 plan_node, plan_size, sql);
#endif
  }
...
public:
  ...
  void DEBUG_REPALY_LOG(const RedoLogRecord &redo_log_hdr) {
    ...
    //     redo_log_hdr.lsn_, redo_log_hdr.log_tid_,
    //     RedoLogTypeStr[redo_log_hdr.log_type_]);
#endif
  }

  void DEBUG_WRITE_LOG(const int lsn_t, bool status, const std::string &log) {
#ifdef PRINT_LOG
    // logger->info("[WRITE LOG][LSN: {}][Status: {}][log: ]", lsn_t, status,
    // log);
#endif
  }
