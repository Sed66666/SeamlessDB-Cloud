...
MetaManager::MetaManager(const std::string &config_path) {
  ...
  local_machine_id =
      (node_id_t)cJSON_GetObjectItem(local_node, "machine_id")->valueint;
  ...
  remote_nodes.push_back(
      RemoteNode{.node_id = 0, .ip = ip->valuestring, .port = port->valueint});
  ...
  // auto remote_ports = state_nodes.get("remote_ports");            // Array
  // Used for RDMA exchanges auto remote_meta_ports =
  // state_nodes.get("remote_meta_ports");  // Array Used for transferring
  // datastore metas
  ...
  //     remote_meta_port); if (remote_machine_id == -1) { std::cerr << "Thread
  //     " << std::this_thread::get_id() << " GetMemStoreMeta() failed!,
  //     remote_machine_id = -1" << std::endl;
  //     }
  //     int remote_port = (int)remote_ports.get(index).get_int64();
  //     remote_nodes.push_back(RemoteNode{.node_id = remote_machine_id, .ip =
...
node_id_t MetaManager::GetMemStoreMeta(std::string &remote_ip,
  ...
  //   strerror(errno); free(recv_buf); close(client_socket); return -1;
  // }
  ...
  //   RDMA_LOG(FATAL) << "remote machine id " << remote_machine_id << " exceeds
  //   the max machine number";
  // }
  ...
  //     sizeof(HashMeta)); primary_hash_metas[meta.table_id] = meta;
...
void MetaManager::GetMRMeta(const RemoteNode &node) {
  ...
  while (QP::get_remote_mr(node.ip, node.port, STATE_TXN_LIST_ID,
                           &remote_txn_list_mr) != SUCC) {
    usleep(2000);
  }
  while (QP::get_remote_mr(node.ip, node.port, STATE_LOCK_BUF_ID,
                           &remote_lock_buf_mr) != SUCC) {
    usleep(2000);
  }
  while (QP::get_remote_mr(node.ip, node.port, STATE_LOG_BUF_ID,
                           &remote_log_buf_mr) != SUCC) {
    usleep(2000);
  }
  while (QP::get_remote_mr(node.ip, node.port, STATE_SQL_BUF_ID,
                           &remote_sql_buf_mr) != SUCC) {
    usleep(2000);
  }
  while (QP::get_remote_mr(node.ip, node.port, STATE_PLAN_BUF_ID,
                           &remote_plan_buf_mr) != SUCC) {
    usleep(2000);
  }
  // join plan & join block
  while (QP::get_remote_mr(node.ip, node.port, STATE_JOIN_PLAN_BUF_ID,
                           &remote_join_plan_buf_mr) != SUCC) {
    usleep(2000);
  }
  while (QP::get_remote_mr(node.ip, node.port, STATE_JOIN_BLOCK_BUF_ID,
                           &remote_join_block_buf_mr) != SUCC) {
    usleep(2000);
  }
