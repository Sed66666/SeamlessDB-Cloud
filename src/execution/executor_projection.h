...
public:
  ...
  // int be_call_times_; //
  // 只需要记录be_call_times，left_child_call_times_应该和当前节点的be_call_times一致
...
public:
  ProjectionExecutor(std::shared_ptr<AbstractExecutor> prev,
                     const std::vector<TabCol> &sel_cols, Context *context,
  ...
  void set_root() {
    is_root_ = true;
    ck_infos_.push_back(ProjectionCheckpointInfo{
  ...
  std::unique_ptr<Record> Next() override {
    ...
    for (size_t proj_idx = 0; proj_idx < proj_cols.size(); proj_idx++) {
      ...
      memcpy(proj_rec->raw_data_ + proj_col.offset,
             prev_rec->raw_data_ + prev_col.offset, proj_col.len);
    }
  ...
  int checkpoint(char *dest) override { return 0; }

  void load_state_info(ProjectionOperatorState *proj_op_state);

  std::pair<bool, double>
  judge_state_reward(ProjectionCheckpointInfo *curr_ck_info);
