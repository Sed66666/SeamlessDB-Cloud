...
public:
  ...
  std::unordered_map<std::string, std::vector<std::unique_ptr<Record>>>
      hash_table_; // left_算子中间结果的hash表
  std::unordered_map<std::string,
                     std::vector<std::unique_ptr<Record>>>::const_iterator
      left_iter_;         // 和右边tuple符合join条件的hash表中的iter
  int left_tuples_index_; // 符合条件的left_records的index（hash表中的vectorindex）
  std::unordered_map<std::string, size_t>
      checkpointed_indexes_; // 上一次检查点最后一次记录的index
...
public:
  ...
  HashJoinExecutor(std::shared_ptr<AbstractExecutor> left,
                   std::shared_ptr<AbstractExecutor> right,
                   std::vector<Condition> conds, Context *context, int sql_id,
                   int operator_id)
    ...
    ck_infos_.push_back(HashJoinCheckpointInfo{
  ...
  bool is_hash_table_built() const { return initialized_; }
  ...
  std::unordered_map<std::string,
                     std::vector<std::unique_ptr<Record>>>::const_iterator
  find_match_join_key(const Record *right_tuple);

  int checkpoint(char *dest) override { return -1; };

  std::pair<bool, double>
  judge_state_reward(HashJoinCheckpointInfo *curr_ck_info);
  ...
  void append_tuple_to_hash_table_from_state(char *src, int left_rec_len,
                                             char *join_key);
