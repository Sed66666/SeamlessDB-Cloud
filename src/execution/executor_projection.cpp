...
void ProjectionExecutor::load_state_info(
    ProjectionOperatorState *proj_op_state) {
...
double ProjectionExecutor::get_curr_suspend_cost() {
  double src_op;
  if (is_root_) {
    src_op = (double)projection_state_size_min +
             (double)(curr_result_num_ - checkpointed_result_num_) * len_;
  } else {
...
std::pair<bool, double> ProjectionExecutor::judge_state_reward(
    ProjectionCheckpointInfo *current_ck_info) {
  ...
  double src_op =
      (double)projection_state_size_min +
      (double)(curr_result_num_ - checkpointed_result_num_) * prev_->tupleLen();
  double rc_op = getRCop(current_ck_info->ck_timestamp_);

  if (rc_op == 0) {
    std::cerr << "[Error]: RCop is 0! [Location]: " << __FILE__ << ":"
              << __LINE__ << std::endl;
    return {false, -1};
  }
  ...
  if (rew_op > 0) {
    if (auto x = dynamic_cast<HashJoinExecutor *>(prev_.get())) {
      current_ck_info->left_rc_op_ = x->getRCop(current_ck_info->ck_timestamp_);
                   ...
                   dynamic_cast<BlockNestedLoopJoinExecutor *>(prev_.get())) {
      current_ck_info->left_rc_op_ = x->getRCop(current_ck_info->ck_timestamp_);
    ...
    RwServerDebug::getInstance()->DEBUG_PRINT(
        "[ProjectionExecutor][op_id: " + std::to_string(operator_id_) +
        "]: [delta result num]: " +
        std::to_string(curr_result_num_ - checkpointed_result_num_) +
...
ProjectionExecutor::get_latest_ckpt_time() {
  return ck_infos_[ck_infos_.size() - 1].ck_timestamp_;
}

int64_t ProjectionExecutor::getRCop(
    std::chrono::time_point<std::chrono::system_clock> curr_time) {
  // TODO:
  // 如果是root的projection，按照当前这个逻辑就可以，如果是非root的projection，直接返回儿子节点的RCop
  if (is_root_ == false) {
    if (auto x = dynamic_cast<HashJoinExecutor *>(prev_.get())) {
  ...
  if (latest_ck_info == nullptr) {
    std::cerr << "[Error]: No ck points found! [Location]: " << __FILE__ << ":"
              << __LINE__ << std::endl;
    assert(0);
  }
...
void ProjectionExecutor::write_state() {
  ProjectionCheckpointInfo curr_ck_info = {
      .ck_timestamp_ = std::chrono::high_resolution_clock::now()};
  double src_op = (double)projection_state_size_min +
                  (double)(curr_result_num_ - checkpointed_result_num_) * len_;
  ...
  if (state_open_) {
    ...
    if (able_to_write) {
      auto [status, actual_size] =
          context_->op_state_mgr_->add_operator_state_to_buffer(this, src_op);
      // RwServerDebug::getInstance()->DEBUG_PRINT("[ProjectionExecutor: " +
      // std::to_string(operator_id_) + "]: [Write State]: [Actual size]: " +
      // std::to_string(src_op));
      if (status) {
        ck_infos_.push_back(curr_ck_info);
...
void ProjectionExecutor::format_collect(
    std::string prefix,
    opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> span) {
  std::string s;
  for (const auto &col : cols_) {
    s.append(col.name).append(", ");
  }
  span->AddEvent(prefix + "投影", {{"列", s}});
  prev_->format_collect("", span);
}