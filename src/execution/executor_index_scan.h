...
private:
  ...
  // std::vector<std::string> index_col_names_;  // index
  // scan涉及到的索引包含的字段
...
public:
  IndexScanExecutor(SmManager *sm_manager, std::string tab_name,
                    ...
                    std::vector<Condition> index_conds, Context *context,
                    int sql_id, int operator_id)
    ...
    for (auto &proj_col : proj_cols) {
      ...
      sel_idxs_.push_back(
          pos -
          tab_cols_.begin()); // 代表的是proj_col在原table的cols中位于第几个
      // std::cout << "sel_idx: " << sel_idxs_.back() << std::endl;
      cols_.push_back(*pos);
      cols_.back().offset =
          curr_offset; // cols_中每个字段的offset代表的是在完成投影后的record中的偏移
    ...
    if (context_ != nullptr) {
      if (is_seq_scan_ == true) {
        lock = context_->lock_mgr_->request_table_lock(
            tab_.table_id_, context_->txn_, LockMode::LOCK_S,
            context_->coro_sched_->t_id_);
        assert(lock != nullptr);
        ...
        case T_Delete: {
          lock = context_->lock_mgr_->request_table_lock(
              tab_.table_id_, context_->txn_, LockMode::LOCK_IX,
              context_->coro_sched_->t_id_);
        } break;
        case T_select: {
          lock = context_->lock_mgr_->request_table_lock(
              tab_.table_id_, context_->txn_, LockMode::LOCK_IS,
              context_->coro_sched_->t_id_);
        } break;
        default:
  ...
  void beginTuple() override {
    ...
    //         if(eval_conds(cols_, filter_conds_, record.get()) &&
    ...
    for (i = 0; i < (int)index_conds_.size() && i < index_meta_.col_num; ++i) {
      ...
      if (cond.op != OP_EQ) {
        ...
        case OP_GT: {
          ...
          // std::cout << "lower_rid: {page_no=" << lower.page_no << ",
          // slot_no=" << lower.slot_no << "}\n"; std::cout << "upper_rid:
          // {page_no=" << upper.page_no << ", slot_no=" << upper.slot_no <<
          // "}\n";
        } break;
        case OP_GE: {
    ...
    */
    ...
    std::cout << "lower_rid: {page_no=" << lower_rid_.page_no
              << ", slot_no=" << lower_rid_.slot_no << ", record_no"
              << lower_rid_.record_no << "}\n";
    std::cout << "upper_rid: {page_no=" << upper_rid_.page_no
              << ", slot_no=" << upper_rid_.slot_no << ", record_no"
              << upper_rid_.record_no << "}\n";
      ...
      if (index_meta_.col_num == (int)index_conds_.size()) {
        ...
        if (lower == upper) {
          ...
          lock = lock_mgr->request_record_lock(
              tab_.table_id_, lower, txn, RECORD_LOCK_GAP,
              get_lock_mode_for_plan(context_->plan_tag_),
              context_->coro_sched_->t_id_);
        } else {
          // record id = x exists, record_not_gap lock
          lock = lock_mgr->request_record_lock(
              tab_.table_id_, lower, txn, RECORD_LOCK_REC_NOT_GAP,
              get_lock_mode_for_plan(context_->plan_tag_),
              context_->coro_sched_->t_id_);
        }
        ...
        lock = lock_mgr->request_record_lock(
            tab_.table_id_, upper, txn, RECORD_LOCK_GAP,
            get_lock_mode_for_plan(context_->plan_tag_),
            context_->coro_sched_->t_id_);
        max_lock_ = true;
      }
    ...
    case OP_GT: {
      ...
      lock = lock_mgr->request_record_lock(
          tab_.table_id_, lower, txn, RECORD_LOCK_ORDINARY,
          get_lock_mode_for_plan(context_->plan_tag_),
          context_->coro_sched_->t_id_);
      min_lock_ = true;
      assert(lock != nullptr);
    ...
    case OP_LE: {
      ...
      if (check_match(cols_, index_conds_, upper_record.get())) {
        ...
        lock = lock_mgr->request_record_lock(
            tab_.table_id_, upper, txn, RECORD_LOCK_ORDINARY,
            get_lock_mode_for_plan(context_->plan_tag_),
            context_->coro_sched_->t_id_);
      } else {
  ...
  NOLOCK1:
    // std::cout << "lower_rid: {page_no=" << lower_rid_.page_no << ", slot_no="
    // << lower_rid_.slot_no << ", record_no" << lower_rid_.record_no << "}\n";
    // std::cout << "upper_rid: {page_no=" << upper_rid_.page_no << ", slot_no="
    ...
    while (!scan_->is_end()) {
      ...
      // if (eval_conds(cols_, index_conds_, rec.get()) && rec->is_deleted() ==
      // false) {
      ...
      if (rec->is_deleted() == false &&
        ...
        if (min_lock_ == false) {
          lock = lock_mgr->request_record_lock(tab_.table_id_, rid_, txn,
                                               RECORD_LOCK_ORDINARY, LOCK_S,
                                               context_->coro_sched_->t_id_);
          assert(lock != nullptr);
          txn->append_lock(lock);
        } else {
  ...
  bool check_match_for_key(const std::vector<ColMeta> &rec_cols,
                           const Condition &cond, const Record *rec) {
  ...
  bool check_match(const std::vector<ColMeta> &rec_cols,
                   const std::vector<Condition> &conds, const Record *rec) {
    for (size_t i = 0; i < conds.size(); ++i) {
      if (!check_match_for_key(rec_cols, conds[i], rec))
        return false;
      if (conds[i].op != OP_EQ)
        return true;
    }
  ...
  void nextTuple() override {
    ...
    assert(!is_end());
    // if(load_from_state_) {
    //     std::cout << "IndexScan Current location: " << rid_.page_no << ", "
    //     << rid_.slot_no << std::endl; load_from_state_ = false;
    // }
    Lock *lock = nullptr;
    auto &tab_cols_ = tab_.cols_;
    for (scan_->next(); !scan_->is_end(); scan_->next()) {
      ...
      if (rec->is_deleted() == false &&
        ...
        if (node_type_ == 0) {
          ...
          lock = context_->lock_mgr_->request_record_lock(
              tab_.table_id_, rid_, context_->txn_, RECORD_LOCK_ORDINARY,
              get_lock_mode_for_plan(context_->plan_tag_),
              context_->coro_sched_->t_id_);
          assert(lock != nullptr);
          context_->txn_->append_lock(lock);
        ...
        assert(context_ != nullptr);
        lock = context_->lock_mgr_->request_record_lock(
            tab_.table_id_, rid_, context_->txn_, RECORD_LOCK_ORDINARY, LOCK_S,
            context_->coro_sched_->t_id_);
        assert(lock != nullptr);
        context_->txn_->append_lock(lock);
      }
  ...
  ColMeta get_col_offset(const TabCol &target) override {
    auto pos =
        std::find_if(cols_.begin(), cols_.end(), [&](const ColMeta &col) {
  ...
  std::unique_ptr<Record> Next() override {
    ...
    for (size_t proj_idx = 0; proj_idx < cols_.size(); ++proj_idx) {
      ...
      memcpy(proj_sel_record->raw_data_ + proj_col.offset,
             current_record_->raw_data_ + prev_col.offset, proj_col.len);
    }
    return proj_sel_record;
    // return std::make_unique<Record>(*current_record_);   //
    // 复制构造，代价稍微高一些
  }
  ...
  std::pair<bool, std::unique_ptr<Record>> mvcc_get_record(Rid rid) {
    ...
    if (ReadView::read_view_sees_trx_id(context_->txn_->get_read_view(),
                                        record_hdr->trx_id_)) {
      return {true, std::move(rec)};
    }
    while (true) {
      Rid old_rid{.page_no = record_hdr->rollback_page_no_,
                  .slot_no = record_hdr->rollback_slot_no_};
      ...
      if (ReadView::read_view_sees_trx_id(context_->txn_->get_read_view(),
                                          record_hdr->trx_id_)) {
        return {true,
                std::make_unique<Record>(old_record->data, old_record->size)};
      }
    }
    return {false, nullptr};
  }
