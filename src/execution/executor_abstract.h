...
public:
  ...
  inline AbstractExecutor(int sql_id = -1, int op_id = -1)
      : exec_type_(ExecutionType::NOT_DEFINED), sql_id_(sql_id),
        operator_id_(op_id), output_count_(0), input_count_(0) {}
  ...
  std::vector<ColMeta>::const_iterator
  get_col(const std::vector<ColMeta> &rec_cols, const TabCol &target) {
    auto pos =
  ...
  get_latest_ckpt_time() = 0;
  virtual double get_curr_suspend_cost() = 0;

  // 从Record中取出某一列的Value
  Value fetch_value(const std::unique_ptr<Record> &record,
  ...
  bool compare_value(const Value &left_value, const Value &right_value,
                     CompOp op) const {
    // 检查type是否一致
    if (!check_type(left_value.type, right_value.type)) {
      throw IncompatibleTypeError(coltype2str(left_value.type),
                                  coltype2str(right_value.type));
    }
    switch (op) {
    case OP_EQ:
