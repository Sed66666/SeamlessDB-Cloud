...
public:
  SeqScanExecutor(SmManager *sm_manager, std::string tab_name,
    ...
    if (context_ != nullptr) {
      Lock *lock = context_->lock_mgr_->request_table_lock(
          tab.table_id_, context_->txn_, LockMode::LOCK_S,
  ...
  void beginTuple() override {
    ...
    while (!scan_->is_end()) {
      rid_ = scan_->rid();
      // std::cout << "SeqScan, rid: {page_no=" << rid_.page_no << ", slot_no="
      // << rid_.slot_no << "}\n";
      auto record = pindex_handle_->get_record(scan_->rid(), context_);
      if (eval_conds(cols_, fed_conds_, record.get()) &&
  ...
  void nextTuple() override {
    ...
    // 用TableIterator遍历TableHeap中的所有Tuple
    //     rid_ = scan_->rid();
