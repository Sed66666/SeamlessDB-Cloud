...
void HashJoinExecutor::beginTuple() {
  ...
  if (!initialized_) {
    ...
    if (is_in_recovery_ == false) {
      ...
      left_->nextTuple();
      std::cout << "HashJoinOp Recovery BeginTuple: left_child_call_times: "
                << left_child_call_times_
                << ", hash_table_size: " << left_hash_table_curr_tuple_count_
                << std::endl;
    }
    ...
    for (; !left_->is_end(); left_->nextTuple()) {
      ...
      // std::cout << "HashJoinFindHashOneTupel time: " <<
      // std::chrono::duration_cast<std::chrono::milliseconds>(find_end -
      // find_start).count() << "ms" << std::endl;
      ...
      // RwServerDebug::getInstance()->DEBUG_PRINT("[HashJoinExecutor::beginTuple,
      // initiate left hash table][operator_id: " + std::to_string(operator_id_)
      // + "][key: " + key + "]");
      ...
      // std::cout << "HashJoinPushOneTupleIntoHashTable time: " <<
      // std::chrono::duration_cast<std::chrono::milliseconds>(find_end -
      // find_start).count() << "ms" << std::endl;
      ...
      if (node_type_ == 1)
        ...
        write_state_if_allow();
      // find_end = std::chrono::high_resolution_clock::now();
      // std::cout << "HashJoinWriteCkpt time: " <<
      // std::chrono::duration_cast<std::chrono::milliseconds>(find_end -
      // find_start).count() << "ms" << std::endl;
    }

    std::cout << "HashJoinOp, op_id: " << operator_id_
              << ", HashTableCount: " << left_hash_table_curr_tuple_count_
              << ", size: " << (int64_t)left_hash_table_curr_tuple_count_ * len_
              << std::endl;
    initialized_ = true;
    delete[] left_key;
  }
  ...
  while (!right_->is_end() &&
         find_match_join_key(right_rec.get()) == hash_table_.end()) {
    right_->nextTuple();
    if (right_->is_end()) {
      is_end_ = true;
      return;
...
void HashJoinExecutor::append_tuple_to_hash_table_from_state(char *src,
                                                             int left_rec_len,
                                                             char *join_key) {
  ...
  // RwServerDebug::getInstance()->DEBUG_PRINT("[HashJoinExecutor::append_tuple_to_hash_table_from_state][operator_id:
  // " + std::to_string(operator_id_) + "][key: " + key + "]");
...
void HashJoinExecutor::nextTuple() {
  ...
  if (left_tuples_index_ < left_iter_->second.size()) {
    // std::cout << "HashJoinExecutor::nextTuple(), left_tuple_index <
    // left_iter->second.size()\n";
    return;
  }
...
std::unique_ptr<Record> HashJoinExecutor::Next() {
  ...
  const std::vector<std::unique_ptr<Record>> &record_vector =
      left_iter_->second;
  const std::unique_ptr<Record> &left_rec =
      left_iter_->second[left_tuples_index_];
  auto right_rec = right_->Next();
  auto res = std::make_unique<Record>(len_);
  memcpy(res->raw_data_, left_rec->raw_data_, left_rec->data_length_);
  memcpy(res->raw_data_ + left_rec->data_length_, right_rec->raw_data_,
         right_rec->data_length_);
...
std::unordered_map<std::string,
                   std::vector<std::unique_ptr<Record>>>::const_iterator
HashJoinExecutor::find_match_join_key(const Record *right_tuple) {
...
HashJoinExecutor::get_latest_ckpt_time() {
...
double HashJoinExecutor::get_curr_suspend_cost() {
  ...
  src_op += (double)left_->tupleLen() *
            (left_hash_table_curr_tuple_count_ -
             latest_ck_info->left_hash_table_curr_tuple_count_);
  return src_op;
}

std::pair<bool, double>
HashJoinExecutor::judge_state_reward(HashJoinCheckpointInfo *curr_ck_info) {
  /*
  ...
  */
  ...
  // TODO:
  // 如果是在left_hash_table_size=0（也就是只有游标在变化的时候），srcop应该等于提供给父亲节点的中间结果的大小
  if (left_hash_table_curr_tuple_count_ ==
      latest_ck_info->left_hash_table_curr_tuple_count_) {
    src_op += (double)len_ *
              (state_change_time_ - latest_ck_info->state_change_time_);
  }
  ...
  if (rew_op > 0) {
    // if create checkpoint in current time, calculate the rc_op(curr_time) for
    // left child and right child
    if (!initialized_) {
      if (auto x = dynamic_cast<HashJoinExecutor *>(left_.get())) {
        curr_ck_info->left_rc_op_ = x->getRCop(curr_ck_info->ck_timestamp_);
      } else if (auto x =
                     dynamic_cast<BlockNestedLoopJoinExecutor *>(left_.get())) {
        curr_ck_info->left_rc_op_ = x->getRCop(curr_ck_info->ck_timestamp_);
    ...
    RwServerDebug::getInstance()->DEBUG_PRINT(
        "[HashJoinExecutor][op_id: " + std::to_string(operator_id_) +
        "]: [delta_hash_table_tuple_count]: " +
        std::to_string(left_hash_table_curr_tuple_count_ -
                       latest_ck_info->left_hash_table_curr_tuple_count_) +
        " [Rew_op]: " + std::to_string(rew_op) + " [state_size]: " +
        std::to_string(src_op) + " [Src_op]: " + std::to_string(new_src_op) +
        " [Rc_op]: " + std::to_string(rc_op) +
        " [State Theta]: " + std::to_string(state_theta_));
    if (state_change_time_ - latest_ck_info->state_change_time_ < 10)
      return {false, -1};
    return {true, src_op};
  }
...
int64_t HashJoinExecutor::getRCop(
  ...
  if (latest_ck_info == nullptr) {
    std::cerr << "[Error]: HashJoinExecutor: No ck points found! [Location]: "
  ...
  if (initialized_ == true) {
    return std::chrono::duration_cast<std::chrono::microseconds>(
               curr_time - latest_ck_info->ck_timestamp_)
        .count();
  }
...
void HashJoinExecutor::write_state() {
  HashJoinCheckpointInfo curr_ckpt_info = {
      .ck_timestamp_ = std::chrono::high_resolution_clock::now(),
      .left_hash_table_curr_tuple_count_ = left_hash_table_curr_tuple_count_};
  HashJoinCheckpointInfo *latest_ck_info = nullptr;
  if (ck_infos_.empty()) {
    std::cerr << "[Error]: Not Implemented! [Location]: " << __FILE__ << ":"
              << __LINE__ << std::endl;
  ...
  latest_ck_info = &ck_infos_[ck_infos_.size() - 1];
  double src_op = hash_join_state_size_min;
  // std::cout << "current_hash_table_count: " <<
  // left_hash_table_curr_tuple_count_ << ", checkpointed count: " <<
  // left_hash_table_checkpointed_tuple_count_ << "\n";
  src_op += (double)left_->tupleLen() *
            (left_hash_table_curr_tuple_count_ -
             latest_ck_info->left_hash_table_curr_tuple_count_);
...
void HashJoinExecutor::write_state_if_allow(int type) {
  ...
  if (state_open_) {
    ...
    if (able_to_write) {
      // std::cout << "curr_ckpt_info.state_change_time_: " <<
      // state_change_time_ << std::endl;
      auto [status, actual_size] =
          context_->op_state_mgr_->add_operator_state_to_buffer(this, src_op);
      // bool status = true;
      // RwServerDebug::getInstance()->DEBUG_PRINT("[HashJoinExecutor][op_id: "
      // + std::to_string(operator_id_) + "]: [Write State]: " +
      // std::to_string(status) + " [Actual Size]: " +
      // std::to_string(actual_size));
      if (status) {
        curr_ckpt_info.ck_timestamp_ =
...
void HashJoinExecutor::load_state_info(HashJoinOperatorState *state) {
  ...
  if (state->right_child_is_join_ == false) {
    if (auto x = dynamic_cast<IndexScanExecutor *>(right_.get())) {
      // x->load_state_info(dynamic_cast<IndexScanOperatorState
      // *>(state->right_child_state_));
      if (state->right_child_state_->finish_begin_tuple_ == false) {
        std::cout << "HashJoinExecutor::load_state_info: IndexScanExecutor "
                     "beginTuple\n";
        x->beginTuple();
        std::cout << "finish begin tuple\n";
      } else {
        x->load_state_info(
            dynamic_cast<IndexScanOperatorState *>(state->right_child_state_));
        x->nextTuple();
      }
      ...
      if (state->right_child_state_->finish_begin_tuple_ == false) {
        ...
        // TODO!!!!!!!!!
        x->load_state_info(
            dynamic_cast<ProjectionOperatorState *>(state->right_child_state_));
        x->nextTuple();
      }
    }
  }
  ...
  HashJoinCheckpointInfo curr_ckpt_info = {
      ...
      .left_hash_table_curr_tuple_count_ = left_hash_table_curr_tuple_count_,
      .left_rc_op_ = 0,
      .state_change_time_ = left_hash_table_curr_tuple_count_ + be_call_times_};
  state_change_time_ =
      left_hash_table_checkpointed_tuple_count_ + be_call_times_;
  ck_infos_.push_back(curr_ckpt_info);
  std::cout << "HashJoinOp, op_id: " << operator_id_
            << ", HashTableCount: " << left_hash_table_curr_tuple_count_
            << "be_call_time: " << be_call_times_
            << ", state_change_time: " << state_change_time_ << "\n";
    ...
    if (state->left_child_is_join_ == false) {
      if (auto x = dynamic_cast<IndexScanExecutor *>(left_.get())) {
        if (state->left_child_state_->finish_begin_tuple_ == false) {
          ...
          x->load_state_info(
              dynamic_cast<IndexScanOperatorState *>(state->left_child_state_));
          x->nextTuple();
        }
      } else if (auto x = dynamic_cast<ProjectionExecutor *>(left_.get())) {
        if (state->left_child_state_->finish_begin_tuple_ == false) {
          ...
          x->load_state_info(dynamic_cast<ProjectionOperatorState *>(
              state->left_child_state_));
          x->nextTuple();
        }
      }
    }
...
void HashJoinExecutor::format_collect(
    std::string prefix,
    opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> span) {
  std::string s;
  for (const auto &cond : fed_conds_) {
    s.append(cond.lhs_col.col_name)
        .append(CompOpString[cond.op])
        .append(cond.rhs_col.col_name)
        .append(", ");
  }
  span->AddEvent(prefix + "HashJoin", {{"join_condition", s}});
  left_->format_collect("join left-", span);
  right_->format_collect("join right-", span);
}