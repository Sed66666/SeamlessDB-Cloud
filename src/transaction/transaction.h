...
public:
  ...
  //     txn_id_(txn_id) { write_set_ = std::make_shared<std::deque<WriteRecord
  //     *>>(); lock_set_ = std::make_shared<std::unordered_set<Lock *>>();
  //     index_latch_page_set_ = std::make_shared<std::deque<Page *>>();
  //     index_deleted_page_set_ = std::make_shared<std::deque<Page*>>();
  //     prev_lsn_ = INVALID_LSN;
  ...
  inline void clear() {
    for (auto &write_record : *write_set_)
      delete write_record;
    write_set_->clear();
  ...
  inline std::shared_ptr<std::unordered_set<Lock *>> get_lock_set() {
    return lock_set_;
  }
  inline void append_lock(Lock *lock) { lock_set_->emplace(lock); }

  inline void set_read_view(std::vector<txn_id_t> &&active_txn_ids,
    ...
    readview_->udpate_readview(txn_id_, std::move(active_txn_ids), min_txn_id,
                               next_txn_id);
  }
  inline std::shared_ptr<ReadView> get_read_view() { return readview_; }

  // inline void set_span(opentelemetry::nostd::shared_ptr<Span> span) {
  //   span_ = span;
  // }

  // inline opentelemetry::nostd::shared_ptr<Span> get_span() { return span_; }

  // inline void set_scope(opentelemetry::nostd::shared_ptr<Scope> scope) {
  //   scope_ = scope;
  // }
...
private:
  ...
  // opentelemetry::nostd::shared_ptr<Span> span_;
  // opentelemetry::nostd::shared_ptr<Scope> scope_;
};
