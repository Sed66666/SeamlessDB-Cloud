...
LockRequestQueue *
LockManager::get_record_request_queue_(const Rid &rid,
                                       LockListInBucket *lock_list) {
...
Lock *LockManager::check_record_lock_conflic(RecordLockType lock_type,
                                             LockMode lock_mode,
    ...
    if (lock_mode == LOCK_S) {
      ...
      while (iterator != nullptr) {
        ...
        if (iterator->is_insert_intention()) {
          return iterator;
        } else if (iterator->get_record_lock_mode(RECORD_LOCK_GAP) &
                   (1 << LOCK_X)) {
          return iterator;
        } else if (iterator->get_record_lock_mode(RECORD_LOCK_ORDINARY) &
                   (1 << LOCK_X)) {
          return iterator;
        } else if (iterator->get_record_lock_mode(RECORD_LOCK_REC_NOT_GAP) &
                   (1 << LOCK_X)) {
          return iterator;
        }
        iterator = iterator->next_;
  ...
  case RECORD_LOCK_GAP: {
    if (lock_mode == LOCK_S) {
      // if there are other write gap/write next_key/insert_intention locks,
      // cannot upgrade lock
      iterator = request_queue->request_queue_->next_;
      while (iterator != nullptr) {
        if (req != nullptr && iterator->trx_id_ == req->trx_id_) {
        ...
        if (iterator->is_gap() &&
            (iterator->get_record_lock_mode(RECORD_LOCK_GAP) & (1 << LOCK_X))) {
          return iterator;
        } else if (iterator->is_next_key_lock() &&
                   (iterator->get_record_lock_mode(RECORD_LOCK_ORDINARY) &
                    (1 << LOCK_X))) {
          return iterator;
      ...
      // if there are other gap/next_key/insert_intention locks, cannot upgrade
      // lock
      iterator = request_queue->request_queue_->next_;
      while (iterator != nullptr) {
        if (req != nullptr && iterator->trx_id_ == req->trx_id_) {
  ...
  case RECORD_LOCK_REC_NOT_GAP: {
    if (lock_mode == LOCK_S) {
      // if there are other write rec_not_gap/write next_key locks, cannot
      // upgrade lock
      iterator = request_queue->request_queue_->next_;
      while (iterator != nullptr) {
        if (req != nullptr && iterator->trx_id_ == req->trx_id_) {
        ...
        if (iterator->is_record_not_gap() &&
            (iterator->get_record_lock_mode(RECORD_LOCK_REC_NOT_GAP) &
             (1 << LOCK_X))) {
          return iterator;
        } else if (iterator->is_next_key_lock() &&
                   (iterator->get_record_lock_mode(RECORD_LOCK_ORDINARY) &
...
Lock *LockManager::upgrade_record_lock_type_mode(
    RecordLockType lock_type, LockMode lock_mode,
    LockRequestQueue *request_queue, Lock *req, int thread_index) {
  std::unique_lock<std::mutex> lock(latch_);

  Lock *tmp =
      check_record_lock_conflic(lock_type, lock_mode, request_queue, req);
  if (tmp != nullptr) {
    ...
    throw TransactionAbortException(req->trx_id_, tmp->trx_id_,
...
Lock *LockManager::request_record_lock(int table_id, const Rid &rid,
                                       Transaction *txn,
                                       RecordLockType lock_type,
                                       LockMode lock_mode, int thread_index) {
  // std::cout << "request_record_lock: table_id: " << table_id << ",
  // rid.record_no = " << rid.record_no << ", lock_type: " <<
  // LockTypeStr[lock_type] << ", lock_mode: " << LockModeStr[lock_mode] <<
  // "trx_id: " << txn->get_transaction_id() << "\n";
  if (node_type_ == 1) {
    return nullptr;
  }
  ...
  switch (lock_type) {
  case RECORD_LOCK_ORDINARY: {
    lock_request->type_mode_ =
        (LOCK_ORDINARY | (1 << (lock_mode + BIT_LOCK_MODE_ORDINARY)));
  } break;
  case RECORD_LOCK_GAP: {
    lock_request->type_mode_ =
        (LOCK_GAP | (1 << (lock_mode + BIT_LOCK_MODE_GAP)));
  } break;
  case RECORD_LOCK_REC_NOT_GAP: {
    lock_request->type_mode_ =
        (LOCK_REC_NOT_GAP | (1 << (lock_mode + BIT_LOCK_MODE_REC_NOT_GAP)));
  } break;
  case RECORD_LOCK_INSERT_INTENTION: {
  ...
  if (lock_table_.count(lock_data_id) == 0) {
    // no locks on the current page, intiate it in lock table and append lock
    // directly
    LockRequestQueue *request_queue =
        new LockRequestQueue(rid.record_no, lock_request);
    lock_table_.emplace(std::piecewise_construct,
                        std::forward_as_tuple(lock_data_id),
                        std::forward_as_tuple());
    LockListInBucket *lock_list_in_page =
        &lock_table_.find(lock_data_id)->second;
    lock_list_in_page->bucket_id_ = rid.record_no / BUCKET_SIZE;
    lock_list_in_page->first_lock_queue_ = request_queue;

    // @STATE
    if (state_open_)
      ContextManager::get_instance()->append_lock_state(lock_request,
                                                        thread_index);
  ...
  LockRequestQueue *request_queue =
      get_record_request_queue_(rid, lock_list_in_page);

  if (request_queue == nullptr) {
    // there are locks on the current page, but no locks on the current record,
    // append lock directly
  ...
  while (req != nullptr) {
    if (req->trx_id_ == txn->get_transaction_id()) {
      if (lock_type == RECORD_LOCK_INSERT_INTENTION &&
          req->is_insert_intention()) {
        delete lock_request;
        return req;
        ...
        delete lock_request;
        lock.unlock();
        return upgrade_record_lock_type_mode(lock_type, lock_mode,
                                             request_queue, req, thread_index);
      }
    }
    req = req->next_;
  }
  ...
  Lock *tmp =
      check_record_lock_conflic(lock_type, lock_mode, request_queue, nullptr);
  if (tmp != nullptr) {
    ...
    throw TransactionAbortException(txn->get_transaction_id(), tmp->trx_id_,
...
Lock *LockManager::upgrade_table_lock_mode(LockMode lock_mode,
                                           LockRequestQueue *request_queue,
                                           Lock *req, int thread_index) {
  std::unique_lock<std::mutex> lock(latch_);
  ...
  while (iterator != nullptr) {
    ...
    if (iterator->is_conflict_table_mode(lock_mode)) {
      throw TransactionAbortException(req->trx_id_,
                                      AbortReason::DEADLOCK_PREVENTION);
    }
...
Lock *LockManager::request_table_lock(int table_id, Transaction *txn,
                                      LockMode lock_mode, int thread_index) {
  // std::cout << "request_table_lock: table_id: " << table_id << ", lock_mode:
  // " << LockModeStr[lock_mode] << "trx_id: " << txn->get_transaction_id() <<
  // "\n";
  std::unique_lock<std::mutex> lock(latch_);
  ...
  lock_request->type_mode_ =
      (LOCK_TABLE | (1U << (lock_mode + BIT_LOCK_MODE_TABLE)));
  ...
  bool conflict = false;
  while (req != nullptr) {
    // std::cout << "req.trx_id: " << req->trx_id_ << ", request_trx_id: " <<
    ...
    if (req->trx_id_ == txn->get_transaction_id()) {
      ...
      if (req->is_conflict_table_mode(lock_mode)) {
        // std::cout << "conflict: req.trx_id=" << req->trx_id_ << ",
        // req.table_mode:" << req->get_table_lock_mode() << "\n";
        conflict = true;
        break;
      }
    }
...
bool LockManager::unlock(Transaction *txn, Lock *lock) {
  ...
  // std::cout << "release_record_lock: table_id: " << lock->table_id_ << ",
  // rid.record_no = " << lock->record_no_ << ", lock_type: " <<
  // LockTypeStr[lock->get_record_lock_type()] << ", lock_mode: " <<
  // lock->get_record_lock_mode_str(lock->get_record_lock_type()) << "trx_id: "
  // << txn->get_transaction_id() << "\n";
  // }
  // else {
  // std::cout << "release_table_lock: table_id: " << lock->table_id_ << ",
  // lock_mode: " << lock->get_table_lock_mode_str() << "trx_id: " <<
  // txn->get_transaction_id() << "\n";
  // }
...
void LockManager::recover_lock_table(Transaction **active_txn_list,
                                     int thread_num) {
  ContextManager::get_instance()->fetch_lock_states(
      &lock_table_, active_txn_list, thread_num);
}