...
void TransactionManager::commit(Transaction *txn, Context *context) {
  ...
  context->log_mgr_->add_log_to_buffer(
      std::move(std::make_unique<CommitLogRecord>(txn->get_transaction_id())));

  context->log_mgr_->write_log_to_storage();

  // commit之后，将事务移出active_txns，txn->get_transaction_id()
  // auto it = std::find(active_txns_.begin(), active_txns_.end(),
  // txn->get_transaction_id()); if (it != active_txns_.end()) {
...
void TransactionManager::abort(Transaction *txn, Context *context) {
  ...
  while (!write_set->empty()) {
    ...
    IxIndexHandle *pindex_handle =
        sm_manager_->primary_index_.at(item->table_name_).get();
    switch (item->wtype_) {
  ...
  context->log_mgr_->add_log_to_buffer(
      std::move(std::make_unique<AbortLogRecord>(txn->get_transaction_id())));
  context->log_mgr_->write_log_to_storage();
...
void TransactionManager::recover_active_txn_lists(Context *context) {
  ContextManager::get_instance()->fetch_active_txns(active_transactions_,
                                                    thread_num_);
}