...
public:
  ...
  WriteRecord(WType wtype, const std::string &tab_name, char *pkey,
              int pkey_len)
  ...
  WriteRecord(WType wtype, const std::string &tab_name, char *pkey,
              int pkey_len, const Record &record)
  ...
  ~WriteRecord() { delete pkey_; }

  WType wtype_;
  std::string table_name_;
  char *pkey_; // we store the pkey to identify the modified record because Rid
               // can change during the transaction's exectuion
  int pkey_len_;
...
public:
  ...
  explicit TransactionAbortException(txn_id_t txn_id, txn_id_t conflict_txn_id,
                                     AbortReason abort_reason)
      : txn_id_(txn_id), conflict_txn_id_(conflict_txn_id),
        abort_reason_(abort_reason) {}

  txn_id_t get_transaction_id() { return txn_id_; }
  txn_id_t get_conflict_txn_id() { return conflict_txn_id_; }
