...
public:
  ...
  Plan(int sql_id = -1, int plan_id = -1)
      : sql_id_(sql_id), plan_id_(plan_id) {}

  virtual ~Plan() = default;
  virtual int serialize(char *dest) = 0; // return the total length of this node
                                         // and its subtrees(recursive)
  ...
  virtual void format_collect(
      std::string prefix,
      opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> span) = 0;
};

class ScanPlan : public Plan {
public:
  ScanPlan(PlanTag tag, int sql_id, int plan_id, SmManager *sm_manager,
           std::string tab_name, std::vector<Condition> filter_conds,
           std::vector<Condition> index_conds, std::vector<TabCol> proj_cols)
      : Plan(sql_id, plan_id) {
    Plan::tag = tag;
  ...
  void format_print() override {
    ...
    for (const auto &cond : index_conds_) {
      std::cout << cond.lhs_col.col_name << CompOpString[cond.op]
                << cond.rhs_val.int_val << ", ";
    }
  ...
  format_collect(std::string prefix,
    ...
    for (const auto &cond : index_conds_) {
      s.append(cond.lhs_col.col_name)
          ...
          .append(std::to_string(cond.rhs_val.int_val))
          .append(",");
    }
    for (const auto &cond : filter_conds_) {
      s.append(cond.lhs_col.col_name)
    ...
    if (Plan::tag == T_IndexScan) {
      span->AddEvent(prefix + "IndexScan",
                     {{"TableName", tab_name_}, {"conds", s}});
    } else {
      span->AddEvent(prefix + "SeqScan",
                     {{"TableName", tab_name_}, {"conds", s}});
    }
  }

  int plan_tree_size() override { return 1; }
  ...
  int serialize(char *dest) {
    ...
    */
    ...
    offset += sizeof(int);
    for (auto &filter_cond : filter_conds_)
      filter_cond.serialize(dest, offset);

    int index_cond_num = index_conds_.size();
    memcpy(dest + offset, &index_cond_num, sizeof(int));
    offset += sizeof(int);
    for (auto &index_cond : index_conds_)
      index_cond.serialize(dest, offset);
  ...
  static std::shared_ptr<Plan> deserialize(char *src, SmManager *sm_manager) {
    ...
    */
    ...
    return std::make_shared<ScanPlan>(tag, sql_id, plan_id, sm_manager,
                                      tab_name_, filter_conds_, index_conds_,
                                      std::vector<TabCol>());
  }
...
public:
  JoinPlan(PlanTag tag, int sql_id, int plan_id, std::shared_ptr<Plan> left,
  ...
  format_collect(std::string prefix,
    ...
    if (Plan::tag == T_NestLoop) {
      span->AddEvent(prefix + "BlockNestedLoopJoin", {{"join_condition", s}});
    } else if (Plan::tag == T_HashJoin) {
  ...
  int serialize(char *dest) override {
    ...
    */
    ...
    for (auto &cond : conds_)
      cond.serialize(dest, offset);

    int off_left =
        0; // the offset of left_ (start from the end of the current operator)
    int off_right =
        0; // the offset of righth_ (start from the end of the current operator)
    // | tot_size of this plan | cond_num | conds | off_left | off_right| ...
    // left_ ... | right_ |
  ...
  static std::shared_ptr<JoinPlan> deserialize(char *src, SmManager *sm_mgr) {
    ...
    */
    ...
    src = src + offset;
    PlanTag left_tag =
        *reinterpret_cast<const PlanTag *>(src + off_left + sizeof(int));
    PlanTag right_tag =
        *reinterpret_cast<const PlanTag *>(src + off_right + sizeof(int));
    ...
    } else if (left_tag == PlanTag::T_SeqScan ||
               left_tag == PlanTag::T_IndexScan) {
    ...
    } else if (right_tag == PlanTag::T_SeqScan ||
               right_tag == PlanTag::T_IndexScan) {
      right_ = ScanPlan::deserialize(src + off_right, sm_mgr);
    }

    return std::make_shared<JoinPlan>(tag, sql_id, plan_id, left_, right_,
                                      conds_);
  }
...
public:
  SortPlan(PlanTag tag, int sql_id, int plan_id, std::shared_ptr<Plan> subplan,
           TabCol sel_col, bool is_desc)
      : Plan(sql_id, plan_id) {
    Plan::tag = tag;
  ...
  int plan_tree_size() override { return 1 + subplan_->plan_tree_size(); }
  ...
  static std::shared_ptr<SortPlan> deserialize(char *src, SmManager *sm_mgr) {
    ...
    */
    ...
    src = src + offset;
    PlanTag subplan_tag =
        *reinterpret_cast<const PlanTag *>(src + off_subplan + sizeof(int));
    std::shared_ptr<Plan> subplan_;

    if (subplan_tag == PlanTag::T_NestLoop) {
      subplan_ = JoinPlan::deserialize(src + off_subplan, sm_mgr);
    } else if (subplan_tag == PlanTag::T_SeqScan ||
               subplan_tag == PlanTag::T_IndexScan) {
      subplan_ = ScanPlan::deserialize(src + off_subplan, sm_mgr);
    }

    return std::make_shared<SortPlan>(tag, sql_id, plan_id, subplan_, sel_col_,
                                      is_desc_);
  }
...
public:
  ProjectionPlan(PlanTag tag, int sql_id, int plan_id,
                 std::shared_ptr<Plan> subplan, std::vector<TabCol> sel_cols)
      : Plan(sql_id, plan_id) {
    Plan::tag = tag;
    subplan_ = std::move(subplan);
  ...
  format_collect(std::string prefix,
    ...
    std::string s;
    for (const auto &col : sel_cols_) {
      s.append(col.col_name).append(", ");
    }
    span->AddEvent(prefix + "Projection", {{"Column", s}});
    subplan_->format_collect("", span);
  }
  ...
  static std::shared_ptr<ProjectionPlan> deserialize(char *src,
                                                     SmManager *sm_mgr) {
    int offset = 0;
    int tot_size = *reinterpret_cast<const int *>(src + offset);
    offset += sizeof(int);
    ...
    */
    ...
    return std::make_shared<ProjectionPlan>(tag, sql_id, plan_id, subplan_,
                                            sel_cols_);
  }
...
public:
  DMLPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::string tab_name,
          int tab_id, std::vector<Value> values, std::vector<Condition> conds,
  ...
  void format_print() override { subplan_->format_print(); }
  ...
  static std::shared_ptr<DMLPlan> deserialize(char *src, SmManager *sm_mgr) {
    ...
    */
    ...
    return std::make_shared<DMLPlan>(tag, subplan_, tab_name_, tab_id_, values_,
                                     conds_, set_clauses_);
  }
...
public:
  DDLPlan(PlanTag tag, std::string tab_name, std::vector<std::string> col_names,
          std::vector<ColDef> cols, std::vector<std::string> pkeys) {
    Plan::tag = tag;
    tab_name_ = std::move(tab_name);
    cols_ = std::move(cols);
  ...
  static std::shared_ptr<DDLPlan> deserialize(char *src, SmManager *sm_mgr) {
    ...
    */
    ...
    return std::make_shared<DDLPlan>(tag, tab_name, tab_col_names_, cols_,
                                     pkeys_);
  }
