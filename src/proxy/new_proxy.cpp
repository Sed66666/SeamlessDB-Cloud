...
void run_proxy(Proxy *proxy) {
  ...
  for (int i = 0; i < proxy->rw_node_thread_num_; ++i) {
    std::cout << "i: " << i << "\n";
    proxy->rw_threads_.emplace_back(
        [&rw_mutex_, &rw_cv_, &conn_closed, i, proxy]() {
          try {
            run_client(proxy->bench_mark_, proxy->rw_node_ip_,
                       proxy->rw_node_port_, i);
...

  {
    ...
    crash_info_map[proxy->rw_node_ip_].last_crash_time.push_back(
