...
int init_tcp_sock(const char *server_host, int server_port) {
  ...
  // if(setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, &sendtimeout,
  ...
  // if(setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &recvtimeout,
...
void send_recv_sql(int sockfd, std::string sql, char *recv_buf,
                   int thread_index, int is_active) {
  int send_bytes;
  // std::string str = "";
  ...
  if (len < 0) {
    ...
    if (is_active)
      brefore_latency[thread_index] =
          std::chrono::duration<double, std::micro>(
              std::chrono::high_resolution_clock::now() -
              breakpoint_txn_start_time[thread_index])
              .count();
...
void run_client(BenchMark *benchmark, std::string remote_ip, int remote_port,
                int thread_index) {
  ...
  send_recv_sql(sockfd, std::to_string(thread_index), recv_buf, thread_index,
                1);
  ...
  // int retry = 0;
  // std::ofstream longtxnfile("../result/longsql/configthread_" +
  // std::to_string(client_num) + "_" + std::to_string(thread_index) +
  // "_sql.txt", std::ios::trunc);
  ...
  while (true) {
    ...
    // std::cout << "client " << thread_index << " finished generate_txn " <<
    // TXN_NUM_PER_THREAD - cnt << "\n"; cnt --; std::cout << "thread " <<
    // thread_index << ", cnt " << cnt << "\n";
    ...
    breakpoint_txn_start_time[thread_index] =
        std::chrono::high_resolution_clock::now();
    for (next_sql_index[thread_index] = 0;
         next_sql_index[thread_index] < txn->queries.size();
         ++next_sql_index[thread_index]) {
      // std::this_thread::sleep_for(std::chrono::seconds(1));
      memset(recv_buf, '\0', MAX_MEM_BUFFER_SIZE);
      send_recv_sql(sockfd, txn->queries[next_sql_index[thread_index]],
                    recv_buf, thread_index, 1);
    ...
    // std::cout << "latency for ol_cnt = 100 " << std::chrono::duration<double,
    // std::milli>(end_time - start_time).count() << "\n"; double latency =
    // aborted ? 0 : std::chrono::duration<double, std::milli>(end_time -
    // start_time).count(); latencies.push_back(latency); if(latency > 1000) {
  ...
  std::cout
      << "ro latency: "
      << std::chrono::duration<double, std::milli>(ro_end - ro_start).count()
      << "\n";
  ...
  // std::ofstream outfile("../result/latency/configthread_" +
  // std::to_string(client_num) + "_" + std::to_string(thread_index) +
  // "_latency.txt", std::ios::trunc); for (const auto& latency : latencies) {
  //     outfile << latency << "\n";
  // }
  // outfile.close();
}

bool reconnect_to_backup_rw(std::string back_ip, int back_port,
                            int thread_index, BenchMark *benchmark) {
  ...
  memset(recv_buf, '\0', MAX_MEM_BUFFER_SIZE);
  send_recv_sql(sock_fd, std::to_string(thread_index), recv_buf, thread_index,
                0);
  ...
  std::cout << "next_sql_index: " << next_sql_index[thread_index]
            << "query size: " << txn->queries.size() << "\n";
  for (; next_sql_index[thread_index] < txn->queries.size();
       ++next_sql_index[thread_index]) {
    memset(recv_buf, '\0', MAX_MEM_BUFFER_SIZE);
    // std::cout << "next_sql_index: " << next_sql_index[thread_index] << ",
    // sql: " << txn->queries[next_sql_index[thread_index]] << "\n";
  ...
  breakpoint_latency[thread_index] =
      std::chrono::duration<double, std::micro>(
          std::chrono::high_resolution_clock::now() -
          breakpoint_txn_start_time[thread_index])
          .count();
  remain_backup_latency[thread_index] =
      std::chrono::duration<double, std::micro>(
          std::chrono::high_resolution_clock::now() - start_second)
          .count();
  ...
  while (true) {
    ...
    for (next_sql_index[thread_index] = 0;
         ...
         ++next_sql_index[thread_index]) {
      memset(recv_buf, '\0', MAX_MEM_BUFFER_SIZE);
      send_recv_sql(sock_fd, txn->queries[next_sql_index[thread_index]],
                    recv_buf, thread_index, 0);
      if (strcmp(recv_buf, "abort\n") == 0) {
        // puts("abort");
        statistics->add_abort_txn_count();
        // retry = 1;
    ...
    // auto end_time = std::chrono::high_resolution_clock::now();
    // double latency = aborted ? 0 : std::chrono::duration<double,
    // std::milli>(end_time - start_time).count(); latencies.push_back(latency);
    // if(latency > 1000) {
    //     for(int i = 0; i < txn->queries.size(); ++i)
    //         longtxnfile << txn->queries[i] << "\n";
    // }
...
void run_proxy(Proxy *proxy) {
  ...
  for (int i = 0; i < proxy->rw_node_thread_num_; ++i) {
    breakpoint_txn_start_time.push_back(
        std::chrono::high_resolution_clock::now());
  ...
  for (int i = 0; i < proxy->rw_node_thread_num_; ++i) {
    ...
    proxy->rw_threads_.emplace_back([&rw_mutex_, &rw_cv_, i, proxy]() {
      try {
        run_client(proxy->bench_mark_, proxy->rw_node_ip_, proxy->rw_node_port_,
                   i);
        ...
        // std::cout << "thread " << i << "encoutners failure,
        // exception_thread_count is " << exception_thread_count << "\n";
      ...
      // //     if(std::all_of(exceptions.begin(), exceptions.end(),
      // [](std::exception_ptr e) { return e != nullptr; })) {
  ...
  //     // std::unique_lock<std::mutex> lock(rw_mutex_);   //
  //     unique_lock会在线程等待期间释放锁, lock_guard就没有这个功能了
  //     // rw_cv_.wait(lock, [&exception_thread_count, proxy] { return
  //     exception_thread_count == proxy->rw_node_thread_num_; });
  ...
  //         //     // std::cerr <<
  //         //     reconnect_to_backup_rw(proxy->back_rw_ip_,
  //         proxy->back_rw_port_);
  //         // }
  //     }
  // }
