...
void StateServer::InitRDMA() {
  ...
  RDMA_ASSERT(rdma_ctrl->register_memory(STATE_LOCK_BUF_ID, lock_buffer,
                                         lock_buf_size,
                                         rdma_ctrl->get_device()) == true);
  RDMA_ASSERT(rdma_ctrl->register_memory(STATE_SQL_BUF_ID, sql_buffer,
                                         sql_buf_size,
                                         rdma_ctrl->get_device()) == true);
  RDMA_ASSERT(rdma_ctrl->register_memory(STATE_PLAN_BUF_ID, plan_buffer,
                                         plan_buf_size,
                                         rdma_ctrl->get_device()) == true);
  /*
      join state
  */
  RDMA_ASSERT(rdma_ctrl->register_memory(STATE_JOIN_PLAN_BUF_ID,
                                         join_plan_buffer, join_plan_size,
                                         rdma_ctrl->get_device()) == true);
  RDMA_ASSERT(rdma_ctrl->register_memory(STATE_JOIN_BLOCK_BUF_ID,
                                         join_block_buffer, join_block_size,
                                         rdma_ctrl->get_device()) == true);
...
int main(int argc, char *argv[]) {
  ...
  size_t txn_list_size =
      ((size_t)cJSON_GetObjectItem(state_node, "txn_list_size_MB")
           ->valuedouble) *
      1024 * 1024;
  size_t log_buf_size =
  ...
  size_t lock_buf_size =
      ...
      1024 * 1024 * 1024;
  size_t sql_buf_size =
      ((size_t)cJSON_GetObjectItem(state_node, "sql_buf_size_MB")->valueint) *
      1024 * 1024;
  size_t plan_buf_size =
      ((size_t)cJSON_GetObjectItem(state_node, "plan_buf_size_GB")->valueint) *
      1024 * 1024 * 1024;
  /*
      join state
  */
  size_t join_plan_size =
      ((size_t)cJSON_GetObjectItem(state_node, "join_plan_buf_size_GB")
           ->valueint) *
      1024 * 1024 * 1024;
  size_t join_block_size =
      ((size_t)cJSON_GetObjectItem(state_node, "join_block_buf_size_GB")
           ->valueint) *
      1024 * 1024 * 1024;

  // cJSON *master_node = cJSON_GetObjectItem(cjson, "master_node");
  // std::string master_node_ip = cJSON_GetObjectItem(master_node,
  // "master_node_ip")->valuestring; cJSON_Delete(cjson);
  ...
  auto server = std::make_shared<StateServer>(
      ...
      sql_buf_size, plan_buf_size, join_plan_size, join_block_size);
  server->AllocMem();
  server->InitMem();
  server->InitRDMA();
