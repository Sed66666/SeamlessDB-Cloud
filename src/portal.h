...
struct PortalStmt {
  ...
  PortalStmt(portalTag tag_, std::vector<TabCol> sel_cols_,
      ...
      : tag(tag_), sel_cols(std::move(sel_cols_)), root(root_),
        plan(std::move(plan_)) {}
};
...
public:
  ...
  std::shared_ptr<PortalStmt> start(std::shared_ptr<Plan> plan,
    ...
    if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
      return std::make_shared<PortalStmt>(
          ...
          std::shared_ptr<AbstractExecutor>(), plan);
      ...
      case T_Update: {
        ...
        std::shared_ptr<AbstractExecutor> root =
            std::make_shared<UpdateExecutor>(sm_manager_, x->tab_name_,
      ...
      case T_Delete: {
        ...
        std::shared_ptr<AbstractExecutor> root =
            std::make_shared<DeleteExecutor>(sm_manager_, x->tab_name_,
      ...
      case T_Insert: {
        std::shared_ptr<AbstractExecutor> root =
            std::make_shared<InsertExecutor>(sm_manager_, x->tab_name_,
                                             x->values_, context);
  ...
  /*
      重做查询
  */
  void re_run(std::shared_ptr<PortalStmt> portal, QlManager *ql,
              Context *context) {
    switch (portal->tag) {
    case PORTAL_ONE_SELECT: {
      ql->re_run_select_from(portal->root, std::move(portal->sel_cols),
                             context);
      break;
    }
  ...
  convert_plan_executor(std::shared_ptr<Plan> plan, Context *context) {
    if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)) {
      return std::make_shared<ProjectionExecutor>(
          convert_plan_executor(x->subplan_, context), x->sel_cols_, context,
          x->sql_id_, x->plan_id_);
    } else if (auto x = std::dynamic_pointer_cast<ScanPlan>(plan)) {
      if (x->tag == T_SeqScan) {
        return std::make_shared<SeqScanExecutor>(sm_manager_, x->tab_name_,
                                                 x->filter_conds_, context);
      } else {
        return std::make_shared<IndexScanExecutor>(
            sm_manager_, x->tab_name_, x->proj_cols_, x->filter_conds_,
            x->index_conds_, context, x->sql_id_, x->plan_id_);
      }
      ...
      if (x->tag == T_NestLoop) {
        return std::make_shared<BlockNestedLoopJoinExecutor>(
            std::move(left), std::move(right), std::move(x->conds_), context,
            x->sql_id_, x->plan_id_);
      } else {
        return std::make_shared<HashJoinExecutor>(
            std::move(left), std::move(right), std::move(x->conds_), context,
            x->sql_id_, x->plan_id_);
      }
    } else if (auto x = std::dynamic_pointer_cast<SortPlan>(plan)) {
      return std::make_shared<SortExecutor>(
          convert_plan_executor(x->subplan_, context), x->sel_col_, x->is_desc_,
          context, x->sql_id_, x->plan_id_);
    }
    return nullptr;
  }

  std::unique_ptr<AbstractExecutor>
  load_op_state(std::shared_ptr<ProjectionPlan> plan, Context *context) {
    /*
    ...
    */
    ...
    return std::make_unique<ProjectionExecutor>(
        convert_plan_executor(plan->subplan_, context), plan->sel_cols_,
        context, plan->sql_id_, plan->plan_id_);
  }
};