...
void sigint_handler(int signo) {
  ...
  int64_t tot_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                             server_end_time - server_start_time)
                             .count();
  ...
  double commit_tput =
      (double)tot_commit_txn / ((double)tot_commit_txn / 1000.0);
  ...
  RwServerDebug::getInstance()->DEBUG_PRINT(
      "[commit_tput: " + std::to_string(commit_tput) +
      "][abort_tput: " + std::to_string(abort_tput) + "]");

  longjmp(jmpbuf, 1);
}

// // 判断当前正在执行的是显式事务还是单条SQL语句的事务，并更新事务ID
// void SetTransaction(txn_id_t *txn_id, Context *context, RWNode* node, int
// connection_id) {
//     // context->txn_ = node->txn_mgr_->get_transaction(*txn_id);
//     if(context->txn_ == nullptr || context->txn_->get_state() ==
//     TransactionState::COMMITTED ||
//         context->txn_->get_state() == TransactionState::ABORTED) {
//         if(context->txn_ != nullptr)
//         node->txn_mgr_->clear_txn(context->txn_); context->txn_ =
//         node->txn_mgr_->begin(nullptr, context->log_mgr_); *txn_id =
...
//                 context->state_slot_index_ =
//                 txn_list_bitmap->get_first_free_bit();
//                 // context->state_slot_index_ =
//                 GetFirstFreeBit(txn_list_bitmap->bitmap_,
//                 txn_list_bitmap->bitmap_size_);
//                 SetBitToUsed(txn_list_bitmap->bitmap_,
//                 context->state_slot_index_); char* txn_list_bitmap_buff =
//                 context->rdma_buffer_allocator_->Alloc(txn_list_bitmap->bitmap_size_);
//                 memcpy(txn_list_bitmap_buff, (char*)txn_list_bitmap->bitmap_,
//                 txn_list_bitmap->bitmap_size_);
//                 if(!context->coro_sched_->RDMAWriteSync(0, qp,
//                 (char*)txn_list_bitmap_buff,
//                 meta_mgr->GetTxnListBitmapAddr(),
//                 txn_list_bitmap->bitmap_size_)) {
//                     RDMA_LOG(ERROR) << "Failed to write txn_list_bitmap into
//                     state_node"; assert(0);
//                 }
//             }
//             TxnItem* txn_item_buf =
//             (TxnItem*)context->rdma_buffer_allocator_->Alloc(sizeof(TxnItem));
...
//             if(!context->coro_sched_->RDMAWriteSync(0, qp,
//             (char*)txn_item_buf,
//             meta_mgr->GetTxnAddrByIndex(context->state_slot_index_),
//             sizeof(TxnItem))) {
//                 RDMA_LOG(ERROR) << "Failed to write txn item into
//                 state_node"; assert(0);
//             }

//             // @TEST
//             context->coro_sched_->RDMAReadSync(0, qp, (char*)txn_item_buf,
//             meta_mgr->GetTxnAddrByIndex(context->state_slot_index_),
//             sizeof(TxnItem)); std::cout << "txn's id: " <<
//             txn->get_transaction_id() << ", txn_item read from state node's
//             id: " << txn_item_buf->id << "\n";
//         }
//     }
// }

RWNode::RWNode(int local_rpc_port, std::string workload, int record_num,
               int thread_num, int buffer_pool_size, std::string config_path)
    : local_rpc_port_(local_rpc_port), workload_(workload),
      record_num_(record_num), thread_num_(thread_num),
  ...
  RDMARegionAllocator::create_instance(MetaManager::get_instance(),
                                       thread_num_);
...
void replay_log_for_resumption(SmManager *sm_mgr) {
  ...
  storage_service::GetPersistLsnResponse *response =
      new storage_service::GetPersistLsnResponse;
  ...
  while (head != tail) {
    ...
    case RedoLogType::UPDATE: {
      UpdateRedoLogRecord *update_redo_log =
          static_cast<UpdateRedoLogRecord *>(redo_log);
      std::string table_name = std::string(update_redo_log->table_name_,
                                           update_redo_log->table_name_size_);
      auto index_handle = sm_mgr->primary_index_[table_name].get();
      if (index_handle == nullptr) {
        throw RMDBError("table name " +
                        std::string(update_redo_log->table_name_) +
                        " not found!");
      }
      index_handle->update_record(update_redo_log->rid_,
                                  update_redo_log->new_value_.data, nullptr);
    } break;
    case RedoLogType::DELETE: {
      DeleteRedoLogRecord *delete_redo_log =
          static_cast<DeleteRedoLogRecord *>(redo_log);
      std::string table_name = std::string(delete_redo_log->table_name_,
                                           delete_redo_log->table_name_size_);
      auto index_handle = sm_mgr->primary_index_[table_name].get();

      index_handle->update_record(delete_redo_log->rid_,
                                  delete_redo_log->delete_value_.data, nullptr);
    } break;
    case RedoLogType::INSERT: {
      InsertRedoLogRecord *insert_redo_log =
          static_cast<InsertRedoLogRecord *>(redo_log);
      std::string table_name = std::string(insert_redo_log->table_name_,
                                           insert_redo_log->table_name_size_);
      auto index_handle = sm_mgr->primary_index_[table_name].get();

      index_handle->replay_insert_record(insert_redo_log->rid_,
                                         insert_redo_log->key_,
                                         insert_redo_log->insert_value_.data);
    } break;
    default:
      break;
    }
...
void client_handler(int *sock_fd, RWNode *node) {
  ...
  */
  ...
  // proxy会为每个compute
  // node维护一个connection_pool，每个compute_node最多同时处理MAX_CONN_LIMIT个客户端链接，
  // 每个客户端连接由proxy分配一个当前compute_node内唯一的connectionid，一旦建立连接，proxy会把connectionid发送给compute
  // node用来标识client
  ...
  */
  ...
  Context *context =
      new Context(node->lock_mgr_, node->log_mgr_, txn, coro_sched,
  ...
  while (true) {
    ...
    if (strcmp(data_recv, "exit") == 0) {
      ...
      if (state_open_ == 1 && node_type_ == 0) {
        // @STATE: prepare for reconnection, the current thread is responsible
        // for the lock recover and log replay
        ...
        // auto recover_duration =
        // std::chrono::duration_cast<std::chrono::microseconds>(recover_end -
        // recover_start).count(); std::cout << "recover_txn_list_time: " <<
        // recover_duration << "\n"; std::cout << "finish recover
        // active_txn_list\n";
        node->lock_mgr_->recover_lock_table(
            node->txn_mgr_->active_transactions_, client_num);
        // recover_end = std::chrono::high_resolution_clock::now();
        // recover_duration =
        // std::chrono::duration_cast<std::chrono::microseconds>(recover_end -
        // recover_start).count(); std::cout << "recover_lock_table_time: " <<
        // recover_duration << "\n"; std::cout << "finish recover lock_table\n";
        replay_log_for_resumption(node->sm_mgr_);
        // recover_end = std::chrono::high_resolution_clock::now();
        // recover_duration =
        // std::chrono::duration_cast<std::chrono::microseconds>(recover_end -
        // recover_start).count(); std::cout << "recover_log_time: " <<
...
*/
        ...
        */
        ...
        std::cout << "Read Op CK meta: checkpoint_num: "
                  << op_ck_meta->checkpoint_num
                  << ", total_size: " << op_ck_meta->total_size
                  << "op_ck_meta->total_src_op: " << op_ck_meta->total_src_op
                  << "\n";
        RwServerDebug::getInstance()->DEBUG_PRINT(
            "[READ OP META FROM STATE][Before][thread id: " +
            std::to_string(op_ck_meta->thread_id) +
            "][checkpoint_num: " + std::to_string(op_ck_meta->checkpoint_num) +
            "][total size: " + std::to_string(op_ck_meta->total_size) +
            "][total srp: " + std::to_string(op_ck_meta->total_src_op));

#ifdef TIME_OPEN
        auto read_state_end = std::chrono::high_resolution_clock::now();
        auto read_state_period =
            std::chrono::duration_cast<std::chrono::microseconds>(
                read_state_end - read_state_start)
                .count();
        std::cout << "time for read state: " << read_state_period << "\n";
        RwServerDebug::getInstance()->DEBUG_PRINT(
            "[time for read state:  " + std::to_string(read_state_period) +
            "]");
#endif
...
*/
#ifdef TIME_OPEN
          auto reconstruct_exec_plan_start =
              std::chrono::high_resolution_clock::now();
#endif

          auto exec_plan = rebuild_exec_plan_from_state(
              node, context, sql_state.get(), op_ck_meta.get(), op_checkpoints);
#ifdef TIME_OPEN
          auto reconstruct_exec_plan_end =
              std::chrono::high_resolution_clock::now();
          auto reconstruct_exec_plan_period =
              std::chrono::duration_cast<std::chrono::microseconds>(
                  reconstruct_exec_plan_end - reconstruct_exec_plan_start)
                  .count();
          std::cout << "time for reconstruct exec plan: "
                    << reconstruct_exec_plan_period << "\n";
          RwServerDebug::getInstance()->DEBUG_PRINT(
              "[time for reconstruct exec plan: " +
              std::to_string(reconstruct_exec_plan_period) + "]");
#endif
...
*/
          ...
          auto re_run_period =
              std::chrono::duration_cast<std::chrono::microseconds>(
                  re_run_end - re_run_start)
                  .count();
          std::cout << "time for re_run: " << re_run_period << "\n";
          RwServerDebug::getInstance()->DEBUG_PRINT(
              "[time for re_run: " + std::to_string(re_run_period) + "]");
#endif
...
#endif
          auto exec_plan =
              rebuild_exec_plan_without_state(node, context, sql_state.get());
...
*/
          ...
          std::cout << "time for re_run: " << re_run_period << "\n";
          RwServerDebug::getInstance()->DEBUG_PRINT(
              "[time for re run: " + std::to_string(re_run_period) + "]");
#endif
        }
#ifdef TIME_OPEN
        ...
        RwServerDebug::getInstance()->DEBUG_PRINT(
            "[time for reconnect: " + std::to_string(reconnect_period) + "]");
#endif
      }
    ...
    //     // @STATE: reconnection, recover the transaction state and continue
    //     to execute
    ...
    //     auto reconnect_end = std::chrono::high_resolution_clock::now();
    //     auto reconnect_period =
    //     std::chrono::duration_cast<std::chrono::microseconds>(reconnect_end -
    //     reconnect_start).count(); std::cout << "time for reconnect: " <<
    //     reconnect_period << "\n";
    // #endif
    // continue;
    // }
    else if (strcmp(data_recv, "reconnect") == 0) {
      // @STATE: reconnection, recover the transaction state and continue to
      // execute
    }
    ...
    if (state_open_) {
      context->op_state_mgr_->write_sql_to_state(sql_id, data_recv,
                                                 i_recvBytes - 1);
    }
...
#ifdef PRINT_LOG
          RwServerDebug::getInstance()->DEBUG_ABORT(
              std::string(data_recv, i_recvBytes), e.GetInfo());
#endif
    ...
    */
    {
      if (state_open_) {
        ...
        // auto op_ck_meta = context->op_state_mgr_->read_op_checkpoint_meta();
        // RwServerDebug::getInstance()->DEBUG_PRINT("[READ OP META FROM
        // STATE][Before][thread id: " + std::to_string(op_ck_meta->thread_id) +
        // "][checkpoint_num: " + std::to_string(op_ck_meta->checkpoint_num) +
        // "][total size: " + std::to_string(op_ck_meta->total_size) + "]");
        ...
        // RwServerDebug::getInstance()->DEBUG_PRINT("[READ OP META FROM
        // STATE][After][thread id: " + std::to_string(op_ck_meta->thread_id) +
        // "][checkpoint_num: " + std::to_string(op_ck_meta->checkpoint_num) +
        // "][total size: " + std::to_string(op_ck_meta->total_size) + "]");
      }
    }
...
void RWNode::start_server() {
  ...
  for (int i = 0; i < client_num; ++i) {
    ...
    // if (pthread_create(&thread_id, nullptr, &client_handler, (void
    // *)(&sockfd), this) != 0) {
  ...
  std::cout << "commit_tot_cnt: " << tot_commit_txn
            << ", time: " << tot_duration << "\n";
  std::cout << "commit_tput: " << commit_tput * 1000.0
            << ", abort_tput: " << abort_tput << "\n";
  ...
  RwServerDebug::getInstance()->DEBUG_PRINT(
      "[commit_tot_cnt: " + std::to_string(tot_commit_txn) +
      "][time: " + std::to_string(tot_duration) + "]");
  RwServerDebug::getInstance()->DEBUG_PRINT(
      "[commit_tput: " + std::to_string(commit_tput * 1000.0) +
      "][abort_tput: " + std::to_string(abort_tput) + "]");
  RwServerDebug::getInstance()->DEBUG_PRINT(
      "[write op checkpoints: " +
      std::to_string(OperatorStateManager::write_cnts) + "]");
}
...
int main(int argc, char **argv) {
  ...
  src_scale_factor_ =
      cJSON_GetObjectItem(node, "src_scale_factor")->valuedouble;
  int buffer_pool_size =
      cJSON_GetObjectItem(node, "buffer_pool_size")->valueint;
  ...
  if (write_ckpt_num == 1)
    ...
    write_ckpt_ = false;
  std::string cost_model_str =
      cJSON_GetObjectItem(node, "cost_model")->valuestring;
  ...
  int sql_buf_size =
      cJSON_GetObjectItem(state_node, "sql_buf_size_GB")->valueint * 1024 *
      1024 * 1024;
  int plan_buf_size =
      cJSON_GetObjectItem(state_node, "plan_buf_size_GB")->valueint * 1024 *
      1024 * 1024;
  int cursor_buf_size =
      cJSON_GetObjectItem(state_node, "cursor_buf_size_GB")->valueint * 1024 *
      1024 * 1024;

  cJSON_Delete(cjson);

  std::cout << "node_id: " << node_id << ", rpc_port: " << local_rpc_port
            << ", workload: " << workload << ", record_num: " << record_num
            << "\n";

  auto server = new RWNode(local_rpc_port, workload, record_num, thread_num,
                           buffer_pool_size, config_path);
