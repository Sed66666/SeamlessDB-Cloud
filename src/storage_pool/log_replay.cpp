...
LogReplay::LogReplay(LogStore *log_store, DiskManager *disk_manager,
                     IxManager *ix_manager, SmManager *sm_manager,
                     ShareStatus *share_status)
    : log_store_(log_store), disk_manager_(disk_manager),
      ix_manager_(ix_manager), sm_manager_(sm_manager),
      share_status_(share_status) {
...
void LogReplay::replay_log() {
  ...
  auto meter = provider->GetMeter("a");
  auto counter = meter->CreateUInt64Counter("log_replay");
  std::map<std::string, std::string> labels = {{"node", "0"}};
  auto labelkv =
      opentelemetry::common::KeyValueIterableView<decltype(labels)>{labels};

  while (true) {
    // std::cout << "LogReplayThread: current_replay_lsn = " <<
    // share_status_->current_replay_lsn_ << ", need_replay_lsn_ = " <<
    // share_status_->need_replay_lsn_ << "\n";
...
void LogReplay::replay_single_log(const std::string &redo_log_str) {
  ...
  case RedoLogType::UPDATE: {
    ...
    if (index_handle == nullptr) {
      throw RMDBError("table name " + std::string(update_redo_log.table_name_) +
                      " not found!");
    }
