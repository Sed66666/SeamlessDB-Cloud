...
StoragePoolImpl::StoragePoolImpl(DiskManager *disk_manager, LogStore *log_store,
                                 ShareStatus *share_status,
                                 BufferPoolManager *buffer_pool_mgr)
    : disk_manager_(disk_manager), log_store_(log_store),
      share_status_(share_status), buffer_pool_manager_(buffer_pool_mgr) {}

StoragePoolImpl::~StoragePoolImpl() {}

void StoragePoolImpl::LogWrite(
  ...
  // std::cout << "receive log_write message from compute node, log_message is:
  // " << request->log() << "\n";
  int len = log_buf.length();
  int off = 0;
  const char *src = log_buf.c_str();
  while (off < len) {
    RedoLogType log_type = *reinterpret_cast<const RedoLogType *>(
        src + off + REDO_LOG_TYPE_OFFSET);
    int lsn = *reinterpret_cast<const lsn_t *>(src + off + REDO_LOG_LSN_OFFSET);
    int log_tot_len =
        *reinterpret_cast<const uint32_t *>(src + off + REDO_LOG_TOTLEN_OFFSET);
    int is_persisit =
        *reinterpret_cast<const bool *>(src + off + REDO_LOG_IS_PERSIST_OFFSET);
    // std::cout << "lsn: " << lsn << ", log_tot_len: " << log_tot_len << ",
    // is_persist: " << is_persisit << "\n";
...
void StoragePoolImpl::GetOldPage(
    ::google::protobuf::RpcController *controller,
  ...
  /**
   * TODO:
   * 优化：这里应该把最新的page放在storage节点的buffer_pool里面，从buffer里面读而不是从磁盘读
   */
  ...
  for (int i = 0; i < request->page_id().size(); ++i) {
    ...
    // std::cout << "table_id: " << table_id << ", page_id: " <<
    // request->page_id()[i].page_no();
    char data[PAGE_SIZE];
    try {
...
void StoragePoolImpl::GetLatestPage(
    ::google::protobuf::RpcController *controller,
   ...
   * 优化：这里应该把最新的page放在storage节点的buffer_pool里面，从buffer里面读而不是从磁盘读
   */
  //    std::cout << "page request: pageid={" <<
  //    request->page_id()[0].table_id() << "," <<
  //    request->page_id()[0].page_no() << "\n";
  ...
  for (int i = 0; i < request->page_id().size(); ++i) {
    ...
    int page_no = request->page_id()[i].page_no();
    // std::cout << "table_id: " << table_id << ", page_id: " <<
    // request->page_id()[i].page_no() << ", lsn: " << lsn << "\n";
    char data[PAGE_SIZE];
    // disk_manager_->read_page(fd, page_no, data, PAGE_SIZE);
    while (lsn > share_status_->current_replay_lsn_) {
      // wait
      // std::cout << "waitingforlogreplay: " << "lsn=" << lsn << "replaylsn="
      // << share_status_->current_replay_lsn_<<"\n";
    }
...
void StoragePoolImpl::GetPersistLsn(
    ::google::protobuf::RpcController *controller,
