...
bool BufferPool::find_victim_page(frame_id_t *frame_id) {
  ...
  // std::cout << "This line is number: " << __FILE__  << ":" << __LINE__ <<
  // std::endl; 1
  // 缓冲池还有freepages(缓冲池未满),即free_list非空,直接从free_list取出一个
  ...
  // std::cout << "This line is number: " << __FILE__  << ":" << __LINE__ <<
  // std::endl; std::cout << "replacer_: " << (char *)replacer_ << std::endl; 2
  // 缓冲池已满，根据LRU策略计算是否有victim frame_id
 ...
 */
void BufferPool::update_page(Page *page, PageId new_page_id,
  ...
  // std::cout << "This line is number: " << __FILE__  << ":" << __LINE__ <<
  // std::endl; 2 更新page table
  page_table_.erase(page->get_page_id()); // 删除页表中原page_id和其对应frame_id
  if (new_page_id.page_no !=
      INVALID_PAGE_ID) { // 注意INVALID_PAGE_ID不要加到页表
    page_table_[new_page_id] =
        new_frame_id; // 新的page_id和其对应frame_id加到页表
  }
...
void BufferPool::fetch_page_from_disk(Page *page, PageId page_id,
                                      frame_id_t frame_id) {
  // 1 如果是脏页，一定要写回磁盘，并且把dirty置为false
  if (page->is_dirty()) {
    disk_manager_->write_page(
        disk_manager_->get_table_fd(page->get_page_id().table_id),
        page->get_page_id().page_no, page->get_data(), PAGE_SIZE);
    page->is_dirty_ = false;
  }
  update_page(page, page_id,
              frame_id); // data置为空，dirty页写入磁盘，然后dirty状态置false
  disk_manager_->read_page(disk_manager_->get_table_fd(page_id.table_id),
                           page_id.page_no, page->get_data(), PAGE_SIZE);
}

void BufferPool::fetch_page_from_rpc(Page *page, PageId page_id,
                                     frame_id_t frame_id) {
  // RwServerDebug::getInstance()->DEBUG_PRINT("[fetch_page_from_rpc][begin][table
  // id: " + std::to_string(page_id.table_id) + ", page no: " +
  // std::to_string(page_id.page_no));
  ...
  storage_service::GetLatestPageResponse *response =
      new storage_service::GetLatestPageResponse;
...
Page *BufferPool::fetch_page(PageId page_id) {
  ...
  if (page_id.page_no == 0)
    std::cout << "try to fetch page, pageid={tabel_id=" << page_id.table_id
  ...
  if (iter != page_table_.end()) {
    ...
    frame_id_t frame_id =
        iter->second; // iter是pair类型，其second是page_id对应的frame_id
  ...
  // disk_manager_->read_page(page->get_page_id().fd,
  // page->get_page_id().page_no, page->get_data(), PAGE_SIZE);
...
bool BufferPool::unpin_page(PageId page_id, bool is_dirty) {
  ...
  if (is_dirty) {
    page->is_dirty_ =
        true; // this logic is NOT equal to: page->is_dirty_ = is_dirty
  }
  return true;
}
...
Page *BufferPool::new_page(PageId *page_id) {
  ...
  // std::cout << "This line is number: " << __FILE__  << ":" << __LINE__ <<
  // std::endl; 2 得到victim frame_id（从free_list或replacer中得到）
  (*page_id).page_no = disk_manager_->allocate_page(disk_manager_->get_table_fd(
      (*page_id)
          .table_id)); // 在fd对应的文件分配一个新的page_id（修改了外部参数*page_id）
  Page *page = &pages_[frame_id]; // 由frame_id得到page
  // std::cout << "This line is number: " << __FILE__  << ":" << __LINE__ <<
  // std::endl;
  if (page->is_dirty()) {
...
bool BufferPool::delete_page(PageId page_id) {
  ...
  // disk_manager_->deallocate_page(page_id);  // This does not actually need to
  // do anything for now
  PageId new_page_id{.table_id = page->get_page_id().table_id,
                     .page_no = INVALID_PAGE_ID};
  update_page(page, new_page_id,
              frame_id);          // 注意此处不要把INVALID_PAGE_ID加到页表
...
bool BufferPool::flush_page(PageId page_id) {
  ...
  Page *page = &pages_[frame_id]; // 由frame_id得到page
  // force_page(page); //
  // 这里不能写成force_page中只刷新脏页，这里就算不是脏的也进行刷新
  disk_manager_->write_page(
      disk_manager_->get_table_fd(page->get_page_id().table_id),
      page->get_page_id().page_no, page->get_data(), PAGE_SIZE);
  page->is_dirty_ = false;
...
void BufferPool::flush_all_pages(int table_id) {
  ...
  for (size_t i = 0; i < pool_size_; i++) {
    Page *page = &pages_[i];
    if (page->get_page_id().table_id == table_id &&
