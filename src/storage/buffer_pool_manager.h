...
public:
  BufferPool(NodeType node_type, size_t pool_size, brpc::Channel *page_channel,
             ...
             SliceMetaManager *slice_mgr = nullptr)
      : node_type_(node_type), pool_size_(pool_size),
        page_channel_(page_channel), disk_manager_(disk_manager),
...
public:
  BufferPoolManager(NodeType node_type, size_t pool_size,
                    brpc::Channel *page_channel,
                    DiskManager *disk_manager = nullptr,
                    SliceMetaManager *slice_mgr = nullptr) {
    disk_manager_ = disk_manager;
    int size_per_pool = pool_size / BUFFER_POOL_NUM;
    for (size_t i = 0; i < BUFFER_POOL_NUM; ++i)
      buffer_pools_[i] = new BufferPool(node_type, size_per_pool, page_channel,
                                        disk_manager, slice_mgr);
  }
  ...
  Page *fetch_page(PageId page_id) {
    return buffer_pools_[page_id.page_no % BUFFER_POOL_NUM]->fetch_page(
        page_id);
  }

  Page *new_page(PageId *page_id) {
    int page_no = disk_manager_->get_fd2pageno(
        disk_manager_->get_table_fd((*page_id).table_id));
    return buffer_pools_[page_no % BUFFER_POOL_NUM]->new_page(page_id);
  }

  bool delete_page(PageId page_id) {
    return buffer_pools_[page_id.page_no % BUFFER_POOL_NUM]->delete_page(
        page_id);
  }

  bool unpin_page(PageId page_id, bool is_dirty) {
    return buffer_pools_[page_id.page_no % BUFFER_POOL_NUM]->unpin_page(
        page_id, is_dirty);
  }
