...
void LogManager::write_log_to_storage() {
  ...
  storage_service::LogWriteResponse *response =
      new storage_service::LogWriteResponse;
  ...
  memcpy(trace_parent_ + 16,
         tracer->GetCurrentSpan()->GetContext().span_id().Id().data(), 8);
  cntl->request_attachment().append(trace_parent_);
#endif
  ...
  int64_t curr_size =
      (curr_tail - curr_head + log_buffer_->buf_size_) % log_buffer_->buf_size_;
  if (curr_size == 0)
    return;
  request.set_log(std::move(
      log_buffer_->get_range_string(curr_head, curr_tail, curr_size)));
  // switch (redo_log->log_type_)
  // {
  //     case RedoLogType::UPDATE: {
  //         auto update_log = reinterpret_cast<UpdateRedoLogRecord
  //         *>(redo_log.get()); update_log->serialize(log_data); break;
  //     }
  //     case RedoLogType::DELETE: {
  //         auto delete_log = reinterpret_cast<DeleteRedoLogRecord
  //         *>(redo_log.get()); delete_log->serialize(log_data); break;
  //     }
  //     case RedoLogType::INSERT: {
  //         auto insert_log = reinterpret_cast<InsertRedoLogRecord
  //         *>(redo_log.get()); insert_log->serialize(log_data); break;
  //     }
  //     default: {
...
void LogManager::make_update_redolog(txn_id_t txn_id, RmRecord &old_record,
                                     RmRecord &new_record, Rid rid,
                                     std::string tab_name, bool is_persist) {
  auto update_redolog = std::make_unique<UpdateRedoLogRecord>(
      txn_id, old_record, new_record, rid, tab_name);
...
void LogManager::make_delete_redolog(txn_id_t txn_id, RmRecord &delete_record,
                                     Rid rid, std::string table_name,
                                     bool is_persist) {
  auto delete_redolog = std::make_unique<DeleteRedoLogRecord>(
      txn_id, delete_record, rid, table_name);
...
void LogManager::make_insert_redolog(txn_id_t txn_id, char *key, int key_size,
                                     RmRecord &insert_record, Rid rid,
                                     std::string tab_name, bool is_persist) {
  auto insert_redolog = std::make_unique<InsertRedoLogRecord>(
      txn_id, key, key_size, insert_record, rid, tab_name);
