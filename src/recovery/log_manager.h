...
class LogManager {
public:
  LogManager(brpc::Channel *log_channel, RDMACircularBuffer *log_buffer)
  ...
  // make redo log
  void make_update_redolog(txn_id_t txn_id, RmRecord &old_record,
                           RmRecord &new_record, Rid rid, std::string tab_name,
                           bool is_persist = false);

  void make_delete_redolog(txn_id_t txn_id, RmRecord &delete_record, Rid rid,
                           std::string tab_name, bool is_persist = false);

  void make_insert_redolog(txn_id_t txn_id, char *key, int key_size,
