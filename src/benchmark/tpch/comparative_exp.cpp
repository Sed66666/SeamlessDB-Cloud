...
void ComparativeExp::init_db_meta() {
  ...
  buffer_pool_mgr_ =
      new BufferPoolManager(NodeType::COMPUTE_NODE, buffer_pool_size_,
                            page_channel_, disk_mgr_, slice_mgr_);
  ...
  log_mgr_ = new LogManager(log_channel_,
                            ContextManager::get_instance()->log_rdma_buffer_);
...
void ComparativeExp::get_table_cond(int table_id,
...
std::string ComparativeExp::get_table_join_col(int tab_id) {
  ...
  case 1: {
    ...
    if (rnd == 1)
      return std::move("n_regionkey");
    else
      return std::move("n_nationkey");
  } break;
  case 2: {
    int rnd = RandomGenerator::generate_random_int(1, 2);
    if (rnd == 1)
      return std::move("c_custkey");
    else
      return std::move("c_nationkey");
  } break;
  case 3: {
    int rnd = RandomGenerator::generate_random_int(1, 2);
    if (rnd == 1)
      return std::move("s_suppkey");
    else
      return std::move("s_nationkey");
  } break;
  case 4: {
    int rnd = RandomGenerator::generate_random_int(1, 2);
    if (rnd == 1)
      return std::move("o_orderkey");
    else
      return std::move("o_custkey");
  } break;
  case 5: {
    int rnd = RandomGenerator::generate_random_int(1, 2);
    if (rnd == 1)
      return std::move("l_suppkey");
    else
      return std::move("l_orderkey");
  } break;
  case 6: {
    int rnd = RandomGenerator::generate_random_int(1, 2);
    if (rnd == 1)
      return std::move("ps_suppkey");
    else
      return std::move("ps_partkey");
  } break;
  case 7: {
...
// void ComparativeExp::get_join_cond(int left_tab_id, int right_tab_id,
// std::vector<Condition>& join_conds) {
//     Condition cond;
//     cond.is_rhs_val = false;
//     cond.lhs_col = TabCol{.tab_name = tables[left_tab_id], .col_name =
//     std::move(get_table_join_col(left_tab_id))}; cond.rhs_col =
//     TabCol{.tab_name = tables[right_tab_id], .col_name =
//     std::move(get_table_join_col(right_tab_id))}; cond.op = OP_EQ;
//     join_conds.push_back(std::move(cond));
// }

#define make_lhs_col(left_tab_id, left_col)                                    \
  cond.lhs_col = TabCol{.tab_name = tables[left_tab_id], .col_name = left_col};
#define make_rhs_col(right_tab_id, right_col)                                  \
  cond.rhs_col =                                                               \
      TabCol{.tab_name = tables[right_tab_id], .col_name = right_col};

int ComparativeExp::get_join_cond(int left_table_range, int right_tab_id,
                                  std::vector<Condition> &join_conds) {
...
std::shared_ptr<Plan>
ComparativeExp::generate_proj_plan(int tab_id, std::shared_ptr<Plan> scan_plan,
                                   Context *context, int &curr_sql_id,
                                   int &curr_plan_id) {
  switch (tab_id) {
  case 0: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "region", .col_name = "r_regionkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "region", .col_name = "r_name"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 1: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "nation", .col_name = "n_nationkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "nation", .col_name = "n_regionkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "nation", .col_name = "n_name"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 2: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "customer", .col_name = "c_custkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "customer", .col_name = "c_nationkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 3: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "supplier", .col_name = "s_suppkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "supplier", .col_name = "s_nationkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 4: {
    ...
    cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_orderkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_orderdate"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_custkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 5: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_suppkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_linenumber"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_orderkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 6: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "partsupp", .col_name = "ps_partkey"}));
    cols.push_back(
        std::move(TabCol{.tab_name = "partsupp", .col_name = "ps_suppkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
  case 7: {
    std::vector<TabCol> cols;
    cols.push_back(
        std::move(TabCol{.tab_name = "part", .col_name = "p_partkey"}));
    return std::make_shared<ProjectionPlan>(
        T_Projection, curr_sql_id, curr_plan_id++, std::move(scan_plan),
        std::move(cols));
  } break;
...
std::shared_ptr<Plan> ComparativeExp::generate_total_proj_plan(
    int table_num, std::shared_ptr<Plan> prev_plan, Context *context,
    int &curr_sql_id, int &curr_plan_id) {
  std::vector<TabCol> cols;
  cols.push_back(
      std::move(TabCol{.tab_name = "region", .col_name = "r_regionkey"}));
  cols.push_back(std::move(TabCol{.tab_name = "region", .col_name = "r_name"}));
  if (table_num == 1)
    goto FINAL_PROJ_PLAN;
  cols.push_back(
      std::move(TabCol{.tab_name = "nation", .col_name = "n_nationkey"}));
  cols.push_back(
      std::move(TabCol{.tab_name = "nation", .col_name = "n_regionkey"}));
  ...
  cols.push_back(
      std::move(TabCol{.tab_name = "lineitem", .col_name = "l_shipdate"}));
  cols.push_back(
      std::move(TabCol{.tab_name = "lineitem", .col_name = "l_suppkey"}));
  cols.push_back(
      std::move(TabCol{.tab_name = "lineitem", .col_name = "l_linenumber"}));
...
FINAL_PROJ_PLAN:
  return std::make_shared<ProjectionPlan>(T_Projection, curr_sql_id,
                                          curr_plan_id++, std::move(prev_plan),
                                          std::move(cols));
}
...
void get_tab_cols(int table_id, std::vector<TabCol> &tab_cols) {
  ...
  case 2: {
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "customer", .col_name = "c_id"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "customer", .col_name = "c_custkey"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "customer", .col_name = "c_nationkey"}));
  ...
  case 4: {
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_id"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_orderkey"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "orders", .col_name = "o_orderdate"}));
  ...
  case 5: {
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_id"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_shipdate"}));
    tab_cols.push_back(
        std::move(TabCol{.tab_name = "lineitem", .col_name = "l_suppkey"}));
...
std::shared_ptr<Plan> ComparativeExp::generate_query_tree(Context *context) {
  ...
  for (int i = 0; i < first_join_table_num; ++i) {
    ...
    if (i == 0) {
      ...
      auto scan_plan = std::make_shared<ScanPlan>(
          T_IndexScan, curr_sql_id_, curr_plan_id_++, sm_mgr_, tables[i],
          filter_conds, index_conds, tab_cols);
      assert(scan_plan != nullptr);
      plan = std::move(scan_plan);
      // plan = generate_proj_plan(i, std::move(scan_plan), context,
      // curr_sql_id_, curr_plan_id_);
    } else {
      assert(plan != nullptr);
      // 后面的表直接放到join executor里
      // join_cond为当前表和随机选取前面已经join的表的固定join条件
      // int left_join_table_id = RandomGenerator::generate_random_int(0, i -
      // 1); get_join_cond(left_join_table_id, i, join_conds);
      ...
      auto scan_plan = std::make_shared<ScanPlan>(
          ...
          filter_conds, index_conds, tab_cols);
      assert(scan_plan != nullptr);
      // auto proj_plan = generate_proj_plan(i, std::move(scan_plan), context,
      // curr_sql_id_, curr_plan_id_); 随机生成 hash join or nestedloop join int
      // rnd = RandomGenerator::generate_random_int(1, 2); plan =
      // std::make_shared<JoinPlan>(T_NestLoop, curr_sql_id_, curr_plan_id_ ++,
      // std::move(plan), std::move(scan_plan), join_conds);
      plan = std::make_shared<JoinPlan>(T_HashJoin, curr_sql_id_,
                                        curr_plan_id_++, std::move(plan),
                                        std::move(scan_plan), join_conds);
    }
  }

  // 只有join_node_num_ >
  // max_table_num的情况下才会进入下面的循环，因此所有的表都已经在当前的query
  // tree出现过了
  for (int i = first_join_table_num; i <= join_node_num_; ++i) {
    if (i % 8 == 0) {
      ...
      plan = generate_total_proj_plan(join_node_num_ + 1, std::move(plan),
                                      context, curr_sql_id_, curr_plan_id_);
    }
    assert(plan != nullptr);
    std::vector<Condition> index_conds;
    std::vector<Condition> filter_conds;
    ...
    // int right_tab_id = RandomGenerator::generate_random_int(0, max_table_num
    // - 1);
    int right_tab_id = i % 8;
    // int left_tab_id = RandomGenerator::generate_random_int(0, max_table_num -
    // 1);
    ...
    auto scan_plan = std::make_shared<ScanPlan>(
        T_IndexScan, curr_sql_id_, curr_plan_id_++, sm_mgr_,
        tables[right_tab_id], filter_conds, index_conds, tab_cols);
    assert(scan_plan != nullptr);
    // auto proj_plan = generate_proj_plan(right_tab_id, std::move(scan_plan),
    // context, curr_sql_id_, curr_plan_id_); int rnd =
    // RandomGenerator::generate_random_int(1, 2); plan =
    // std::make_shared<JoinPlan>(T_NestLoop, curr_sql_id_, curr_plan_id_ ++,
    // std::move(plan), std::move(proj_plan), join_conds);
    plan = std::make_shared<JoinPlan>(T_HashJoin, curr_sql_id_, curr_plan_id_++,
                                      std::move(plan), std::move(scan_plan),
                                      join_conds);
  }
  ...
  plan =
      std::make_shared<SortPlan>(T_Sort, curr_sql_id_, curr_plan_id_++,
                                 std::move(plan), std::move(order_col), false);

  // 最后再进行一次projection
  plan = generate_total_proj_plan(join_node_num_ + 1, std::move(plan), context,
                                  curr_plan_id_, curr_sql_id_);

  plan->format_print();

  plan = std::make_shared<DMLPlan>(
      T_select, std::move(plan), std::string(), 0, std::vector<Value>(),
...
void ComparativeExp::normal_exec() {
  std::cout << "************************ normal_exec ************************"
            << std::endl;
  auto normal_start = std::chrono::high_resolution_clock::now();

  int connection_id = 0;
  CoroutineScheduler *coro_sched =
      new CoroutineScheduler(connection_id, CORO_NUM);
  auto local_rdma_region_range =
      RDMARegionAllocator::get_instance()->GetThreadLocalRegion(connection_id);
  ...
  OperatorStateManager *op_state_manager =
      new OperatorStateManager(connection_id, coro_sched, meta_mgr, qp_mgr);
  char *result_str = new char[BUFFER_LENGTH];
  int offset = 0;
  Context *context =
      new Context(lock_mgr_, log_mgr_, txn, coro_sched, op_state_manager,
                  qp_mgr, result_str, &offset, rdma_allocated);
  ...
  auto normal_duration =
      std::chrono::duration_cast<std::chrono::duration<double>>(normal_end -
                                                                normal_start);

  // print_char_array(result_str, offset);

  std::cout << "time for normal exec: " << normal_duration.count() << "s"
            << std::endl;
  std::cout << "write op checkpoints: " << OperatorStateManager::write_cnts
            << "\n";
  std::cout << "write op checkpoint size(Bytes): "
            << OperatorStateManager::write_tot_size << "\n";
...
void ComparativeExp::re_exec() {
  ...
  if (op_ck_meta->checkpoint_num != 0) {
    ...
    auto op_checkpoints =
        context->op_state_mgr_->read_op_checkpoints(op_ck_meta.get());
    rebuild_exec_plan_with_query_tree(context, portal_stmt, op_ck_meta.get(),
                                      op_checkpoints);
    portal_->re_run(portal_stmt, ql_mgr_, context);
  } else {
    RwServerDebug::getInstance()->DEBUG_PRINT(
        "Execute SQL from the beginning\n");
    portal_->run(portal_stmt, ql_mgr_, context);
  }
...
int main(int argc, char **argv) {
  /*
  整体执行流程：执行normal_time *
  break_percentage秒之后，kill掉正常执行的进程，进行恢复
  */
  ...
  std::string cost_model_type =
      cJSON_GetObjectItem(comp_exp_config, "cost_model")->valuestring;
  node_type_ = cJSON_GetObjectItem(comp_exp_config, "node_type")->valueint;
  int buffer_pool_size =
      cJSON_GetObjectItem(comp_exp_config, "buffer_pool_size")->valueint;
  int thread_num = cJSON_GetObjectItem(comp_exp_config, "thread_num")->valueint;
  double normal_time =
      cJSON_GetObjectItem(comp_exp_config, "normal_time")->valuedouble;
  double break_percentage =
      cJSON_GetObjectItem(comp_exp_config, "break_percentage")->valuedouble;

  state_open_ = cJSON_GetObjectItem(comp_exp_config, "state_open")->valueint;
  state_theta_ =
      cJSON_GetObjectItem(comp_exp_config, "state_theta")->valuedouble;
  src_scale_factor_ =
      cJSON_GetObjectItem(comp_exp_config, "src_scale_factor")->valuedouble;
  ...
  int write_ckpt_num =
      cJSON_GetObjectItem(comp_exp_config, "write_ckpt")->valueint;
  if (write_ckpt_num == 1)
    write_ckpt_ = true;
  else
    write_ckpt_ = false;
  ...
  ComparativeExp *comparative_exp =
      new ComparativeExp(join_num, buffer_pool_size, thread_num);

  std::thread normal_thread(&ComparativeExp::normal_exec, comparative_exp);

  std::this_thread::sleep_for(
      std::chrono::duration<double>(normal_time * break_percentage));
  ...
  if (normal_thread.joinable())
    normal_thread.join();

  // memset(test_buffer, 0, bytes_num);
  // CoroutineScheduler* coro_sched = new CoroutineScheduler(0, CORO_NUM);
  // auto rdma_region =
  // RDMARegionAllocator::get_instance()->GetThreadLocalJoinBlockRegion(0);
  // MetaManager* meta_mgr = MetaManager::get_instance();
  // QPManager* qp_mgr = QPManager::get_instance();
  // OperatorStateManager* op_state_manager = new OperatorStateManager(0,
  // coro_sched, meta_mgr, qp_mgr); RCQP* qp =
  // qp_mgr->GetRemoteJoinBlockBufQPWithNodeID(0); size_t remote_offset =
  // meta_mgr->GetJoinBlockAddrByThread(0);
  ...
  // // RCQP* qp =
  // QPManager::get_instance()->GetRemoteJoinBlockBufQPWithNodeID(0);

  // if(!coro_sched->RDMAReadSync(0, qp, rdma_region.first, remote_offset,
  // bytes_num)) {
  ...
  // if(!coro_sched->RDMAWriteSync(0, qp, rdma_region.first + bytes_num,
  // remote_offset, bytes_num)) {
  ...
  // std::cout << "time for memcpy 1MB: " <<
  // std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()
  // << " miu s" << std::endl;

  // std::cout << "time for RDMAWrite 1MB: " <<
  // std::chrono::duration_cast<std::chrono::microseconds>(rdma_write_end -
  // end).count() << " miu s" << std::endl;

  // std::cout << "time for RDMARead 1MB: " <<
  // std::chrono::duration_cast<std::chrono::microseconds>(rdma_read_end -
  // rdma_write_end).count() << " miu s" << std::endl;
  return 0;
}